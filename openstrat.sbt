...
def geomSett = List(
  Compile/unmanagedSourceDirectories ++= List("srcLines", "srcShapes", "src3d", "srcTrans", "srcWeb", "srcGui").map(s => (ThisBuild/baseDirectory).value / "Geom" / s),
)
...
lazy val Tiling = mainJvmProj("Tiling").dependsOn(Geom).settings(tilingSett)
