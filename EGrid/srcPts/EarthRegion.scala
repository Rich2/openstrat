...
{ import pEurope.*
  override val ePolys: RArr[EarthPoly] = RArr(RussiaSouth, VolgaRegion, Caspian, AzovSea, GreeceNorth, GreeceCentral, Euboea, BalkansWest, BalkansEast,
    FennoNorth, FinlandSouth, Karelia, LakePeipus, RussiaNE, LakeLagoda, LakeOnega, KolaPeninsula, Gotland, Saaremaa, Hiiumaa, Crimea, MarmaraSea, Peloponnese)
}
...
{ import pnAmer.*
  override val ePolys: RArr[EarthPoly] = RArr(Baja, Mexico, MexicoEast, Honduras, Nicaragua, LakeCocibolca, CostaRico, Panama, Cuba, Hispaniola, Jamaica)
}
...
{ import soceans.*
  override val ePolys: RArr[EarthPoly] = RArr(WesternAustralia, AustraliaNorthTerr, Queensland, SouthAustraliaWest, LakeEyre, SouthAustraliaEast, NewSouthWales,
    Tasmania, NewCaldedonia, NZNorthIsland, NZSouthIsland)
}
...
{ import pMalay.*
  override val ePolys: RArr[EarthPoly] = RArr(Sumatra, BorneoNorth, BorneoSouth, Sulawesi, Halmahera, BuruIsland, SeramIsland, javaIsland, BaliIsland, Sumbawa,
    Lambok, Sumba, Flores, AlorIsland,  Wetar, Timor, GuineaWest, PapuaNewGuinea, NewBritain, NewIreland, Bougainville, SolomonMiddle, SolomonSE, Luzon,
    Palawan, VisayasWest, SamarLeyte, Mindano)
}
