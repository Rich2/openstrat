...
case class HSysProjectionEarth(parent: EGridSys, panel: Panel) extends HSysProjection
{
  ...
  { val m3 = parent.hCoordLL(hc).toMetres3
    ...
    opt.map(_.mapScalars(metresPerPixel))
  }
  ...
  { val p1 = hc.hVertPolygon.map(parent.hCoordLL(_)).toMetres3.fromLatLongFocus(focus)
    ...
    opt.map{poly => poly.map(_.rotate180If(southUp).mapScalars(metresPerPixel)) }
  }
  ...
  def irrFills: RArr[PolygonFill] = irr1.map { pair =>
    ...
    p.map(_.mapScalars(metresPerPixel)).fill(col)
  }

  def irrLines: RArr[PolygonDraw] = irr1.map { a => a._2.map(_.mapScalars(metresPerPixel)).draw(lineColour = Violet) }

  def irrLines2: GraphicElems = ifTileScale(8, ife(irrOn, irrLines, RArr()))

  def irrActives = irr1.map { a => a._2.map(_.mapScalars(metresPerPixel)).active(a._1) }

  def irrActives2: GraphicElems = ifTileScale(8, ife(irrOn, irrActives, RArr()))

  def irrNames: RArr[TextFixed] = irr1.map { pair =>
    val (d, _) = pair
    val posn = d.cen.toMetres3.fromLatLongFocus(focus).xy.rotate180If(southUp).mapScalars(metresPerPixel)
