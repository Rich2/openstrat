...
case class HSysProjectionEarth(parent: EGridSys, panel: Panel) extends HSysProjection
{
  ...
  override def setView(view: Any): Unit = view match
  {
    ...
    { pixelsPerC = hv.pixelsPerC
      focus = parent.hCoordLLDirn(hv.hCoord, hv.northUp)
    }
  ...
  def irr0: RArr[(EarthArea, PolygonM2)] = eas.map(_.withPolygonM2(focus))
  def irr1: RArr[(EarthArea, PolygonM2)] = irr0.filter(_._2.vertsMin3)
  ...
  def irrActives = irr1.map { a => a._2.map(_ / metresPerPixel).active(a._1) }
