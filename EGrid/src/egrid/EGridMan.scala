...
{ /** The multi grid system that contains the grid that this is managing. */
  ...
  final def vertToCoordFind(hCen: HCen, vertNum: Int, dirn: HVDirn): Option[HCoord] = vertToCoordFind(hCen.r, hCen.c, vertNum, dirn)

  def vertToCoordFind(r: Int, c:Int, vertNum: Int, dirn: HVDirn): Option[HCoord]
}