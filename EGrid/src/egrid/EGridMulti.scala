...
{ ThisMulti =>
  ...
  def gridMansMap[A, AA <: Arr[A]](f: ManT => A)(implicit build: BuilderMapArr[A, AA]): AA = gridMans.map(f)

  def gridMansFlatMap[AA <: Arr[_]](f: ManT => AA)(implicit build: BuilderFlatArr[AA]): AA = gridMans.flatMap(f)
