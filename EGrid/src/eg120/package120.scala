...
package object eg120
{
  ...
  def fullTerrsHCenLayerSpawn(implicit subSys: EGrid120LongMulti): LayerHcSys[WTile] = iToMap(0, subSys.numGrids - 1) { i =>
    val ft = fullTerrs((i + subSys.headGridInt) %% 12)
    ft.terrs.spawn(ft.grid, subSys.grids(i))
  }.combine

  def fullTerrsSideLayerSpawn(implicit subSys: EGrid120LongMulti): HSideOptLayer[WSide, WSideSome] =
