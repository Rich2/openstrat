...
{ type ThisT <: SimilarPreserve
  ...
  def reflect(lineSeg: Sline): ThisT = fTrans(_.reflect(lineSeg))
...
{ def reflectT(obj: T, line: Line): T
  def reflectSegT(obj: T, lineSeg: Sline): T
...
object TransSim
{
  ...
  { override def rotateRadians(obj: T, radians: Double): T = obj.rotateRadians(radians).asInstanceOf[T]
    ...
    override def reflectSegT(obj: T, line: Sline): T = obj.reflect(line).asInstanceOf[T]
  ...
  { override def slate(obj: AA, offset: Vec2): AA = obj.map(ev.slate(_, offset))
    ...
    override def reflectSegT(obj: AA, line: Sline): AA = obj.map(ev.reflectSegT(_, line))
  ...
  { override def slate(obj: Array[A], offset: Vec2): Array[A] = obj.map(ev.slate(_, offset))
    ...
    override def reflectSegT(obj: Array[A], line: Sline): Array[A] = obj.map(ev.reflectSegT(_, line))
...
{ def reflect(line: Line) = ev.reflectT(value, line)
  def reflect(lineSeg: Sline): T = ev.reflectSegT(value, lineSeg)
