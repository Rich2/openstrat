...
class HexGridIrr(override val yTileMin: Int, val indexArr: Array[Int]) extends HexGrid
{
  ...
  def numOfTiles: Int = iToFoldInt(yTileMin, yTileMax, 2) { (acc, i) => acc + cRowLen(i) }
  def cRowStart(y: Int): Int = indexArr(y - yTileMin)
  def cRowLen(y: Int): Int = indexArr(y - yTileMin + 1)
  def cRowEnd(y: Int): Int = cRowStart(y) + cRowLen(y) * 4 - 4

  def foreachRoord(f: Roord => Unit): Unit = ForeachRow{ y => iToForeach(cRowStart(y), cRowEnd(y), 4) { c => f(Roord(y, c)) } }
