...
class TilesOptRef[A <: AnyRef](val unsafeArr: Array[A]) extends AnyVal
{
  ...
  def mutSetSome(y: Int, c: Int, value: A)(implicit grid: TileGridSimple): Unit = unsafeArr(grid.index(y, c)) = value

  def mutSetSome(r: Roord, value: A)(implicit grid: TileGridSimple): Unit = unsafeArr(grid.index(r)) = value
  def mutSetNone(r: Roord)(implicit grid: TileGridSimple): Unit = unsafeArr(grid.index(r)) = null.asInstanceOf[A]
