...
case class DisplayRowGraphicSubject(margin: Double, subjs: Refs[GraphicParentOld])
{ /** Horrible code. Needs rewriting. */
  def fromLeft(leftPt: Vec2): Refs[GraphicParentOld] = subjs.mapWithAcc(leftPt.x + margin)((head, x) =>
