...
object PolyParent
{
  def fill(cen: Vec2, poly: Polygon, evObj: AnyRef, colour: Colour): PolyParent = new PolyParent(cen, poly, evObj, Refs(poly.fill(colour)))

  /** Not sure if this is double filling the polygon */
  def fillDraw(cen: Vec2, poly: Polygon, evObj: AnyRef, fillColour: Colour, lineWidth: Double, lineColour: Colour = Black): PolyParent =
    new PolyParent(cen, poly, evObj, Refs(PolyFillDraw(poly, fillColour, lineWidth, lineColour)))

  def draw(cen: Vec2, poly: Polygon, evObj: AnyRef, lineWidth: Double, lineColour: Colour = Black): PolyParent =
  ...
  def fillContrastText(cen: Vec2, poly: Polygon, evObj: AnyRef, fillColour: Colour, str: String, fontSize: Int = 4): PolyParent =
