...
trait CanvasPlatform extends RectCenlign
{
  ...
  final def polyFill(poly: PolygonTr, colour: Colour): Unit = oif(poly.length >= 3, pPolyFill(poly, colour))
  def pPolyFill(poly: PolygonTr, colour: Colour): Unit
  
  final def polyDraw(poly: PolygonTr, lineWidth: Double, colour: Colour): Unit = oif(poly.length >= 2, pPolyDraw(poly, lineWidth, colour))
  def pPolyDraw(poly: PolygonTr, lineWidth: Double, colour: Colour): Unit
  ...
  final def linesDraw(lineWidth: Double, linesColour: Colour, lines: Sline *): Unit = linesDraw(LinesDraw(Slines(lines: _*), lineWidth, linesColour))
  ...
  def circleFillOld(cf: CircleFill): Unit
  ...
  def ellipseFill(cf: EllipseFill): Unit
  ...
  def rendElems(elems: Arr[DisplayElem]): Unit = elems.foreach(_.rendToCanvas(this))
}