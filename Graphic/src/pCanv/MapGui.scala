...
abstract class MapGui(title: String) extends CanvasPanelled(title)
{
  ...
  def cmdButton(str: String, cmd: AnyRef) = Rectangle.curvedCornersCentred(50, 25, 5).parentAllOld(cmd, White, 3, Black, 25, str)
  
  def clickButton(str: String, cmd: MB0, backColour: Colour = Colour.White) =
    Rectangle.curvedCornersCentred(str.length.max(2) * 17, 25, 5).parentAllOld(MButtonCmd(cmd), backColour, 3, backColour.contrastBW, 25, str)
   
  def buttonStd(str: String, cmd: MB0, backColour: Colour = Colour.White) =
    Rectangle.curvedCornersCentred(100, 25, 5).parentAllOld(cmd, backColour, 3, backColour.contrastBW, 20, str)
  ...
  def reTop(commands: Refs[GraphicParentOld]): Unit = topPan.repaint(DisplayRowGraphicSubject(10, commands).fromLeft(topPan.cenLeft))
