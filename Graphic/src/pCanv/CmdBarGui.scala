...
abstract class CmdBarGui(title: String) extends CanvasPanelled(title)
{
  ...
  /**  repaints the top command bar */
   def reTop(commands: Refs[GraphicParentOld]): Unit = topBar.repaint(DisplayRowGraphicSubject(10, commands).fromLeft(topBar.cenLeft))
}

object StdButton
{
  def apply(str: String, cmd: AnyRef) =
      Rectangle.curvedCornersCentred(str.length.max(2) * 17, 25, 5).parentAllOld(cmd, White, 3, Black, 25, str)
