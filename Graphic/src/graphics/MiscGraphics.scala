...
object Star5
{
  ...
  def apply(ratio: Double = classicRatio): PolygonClass =
  ...
  def ptUpYCentred(ratio: Double = classicRatio): PolygonClass = apply(ratio).slateY(deg36.cos / 2 - 0.5)
}
...
object Star3
{
  def apply(sideLength: Double = 1): PolygonClass =
