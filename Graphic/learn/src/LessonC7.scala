...
{ /** defines the size of the circles that represent the points of the bezier */
  ...
  def drawBezier():Unit =
  { val dragCircles = bezierPoints.map(dc => Circle(dc.loc, circleRadius).fill(dc.color))
    ...
    val quadraticDragCircles = quadraticBezierPoints.map(dc => Circle(dc.loc, circleRadius).fill(dc.color))
