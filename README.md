...
Scala currently set to 2.13.0, except for Scala-native build set to 2.11.12. Sbt currently set to 1.3.0-RC5. Update your Mill to 0.5.0. Run:
...
The tilde **~** tells sbt to rerun the command every time you modify and save a source file. The first command will build and launch a ScalaFx window. It will rebuild and relaunch so you can immediately see the effects of your changes. Copy the AppNum.txt from the Misc folder to the  ~AppData/Local/OpenStratData/Dev folder. Creating directory if not already existing. Change the number in appNum.txt to change the application. All the examples on the richstrat.com website are available plus others. The second command will also rebuild on source changes in similar manner. However unlike with the reStart command, when you make a source file edit and save it, you will have to manually refresh the browser window after the fastOptJS command has finished the rebuild.
