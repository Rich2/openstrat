...
{      
   ...
   {
      val rad: Double = 0.55
      val circ3: PolygonImp = PolygonImp(0 pp rad, - rad * Sin60 pp - rad * Sin30, rad * Sin60 pp - rad * Sin30).scale(0.5)
      val c3 = circ3.ySlate(0.06).map(cen => Circle(2 * rad * 0.46, cen).fill(Black))
      val rect: PolygonFill = TrapezoidIsosceles(0.35, 0.2, 0.5).ySlate(- 0.28).fill(Black)
      c3 +- rect
   }      
