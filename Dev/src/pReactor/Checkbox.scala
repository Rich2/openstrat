...
case class Checkbox(aIsSelected:Boolean = false, labelText:String = "", loc:Pt2 = 0 pp 0, aIsEnabled:Boolean = false, action: (Checkbox) => Unit = (Checkbox) => {}, activeId:Any = "Checkbox") {
  val defaultSize = 12
  var isSelected = aIsSelected
  var isEnabled = aIsEnabled

  def put(aIsSelected: Boolean = isSelected, labelText: String = labelText, loc: Pt2 = loc, aIsEnabled:Boolean = isEnabled, aAction: (Checkbox) => Unit = action, activeId: Any = activeId): GraphicElems =
