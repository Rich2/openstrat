...
case class ReactorGUI (canv: CanvasPlatform) extends CanvasNoPanels("Reactor")
{
  ...
  def drawBalls(loc:Vec2, color:Colour, count:Int) : Unit =
  {
    ...
    if (count==2||count==4||count==5) canv.circleFillOld(CircleFill(Circle(size/8, loc+((size/4) vv (size/4))), color))
    if (count==1||count==3||count==5) canv.circleFillOld(CircleFill(Circle(size/8, loc+((size/2) vv (size/2))), color))
    if (count==2||count==4||count==5) canv.circleFillOld(CircleFill(Circle(size/8, loc+((3*size/4) vv (3*size/4))), color))
    if (count==3||count==4||count==5) canv.circleFillOld(CircleFill(Circle(size/8, loc+((3*size/4) vv (size/4))), color))
    if (count==3||count==4||count==5) canv.circleFillOld(CircleFill(Circle(size/8, loc+((size/4) vv (3*size/4))), color))
