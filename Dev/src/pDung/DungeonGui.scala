...
case class DungeonGui(canv: CanvasPlatform, scen: DungeonScen) extends CmdBarGui("Dungeon Gui")
{
  ...
  val players = scen.characs.mapSomes { (r, cp) =>
    val poly1: PolygonClass = Rectangle(1.5).insVerts(1, -0.25 vv 0.5, 0 vv 0.8, 0.25 vv 0.5).rotate(cp.facing.angle)
    val poly2: PolygonClass = poly1.scale( 0.75).slate(r.gridVec2)
  ...
  thisTop()
  def frame: GraphicElemFulls = (tiles +- sls ++ players).gridScale(scale)
