...
object Rcom
{
   def apply(): List[StratPage]  = List(IndexPage, WWIIPage, Y1783Page, BC305Page, DungeonPage, PlanetPage,
         BrowserTestPage, AboutPage, ZugPage, CivPage, DraughtsPage)
...
import Colour._
   ...
   def toDoEmp(items: Seq[String], title: String, colour: Colour): Seq[HtmlEl] =
      items.ifEmpty(Seq (HtmlH3(title :- "None")), Seq(HtmlH3(title), roadList(items, colour)))   
}
