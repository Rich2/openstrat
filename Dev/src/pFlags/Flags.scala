...
{ def name: String
  ...
  def topToBottom(colours: Colour*): GraphicElems = colours.iMap((colour, i) => Rect.tl(ratio,
     1.0 / colours.length, -ratio / 2 pp + 0.5).slate(0,
...
{ val name = "CommonShapesInFlags"
  ...
  val apply: GraphicElems = Arr(
    ...
    Star5().scale(0.1).slate(0.3 pp 0.3).draw(Lime, 1),
    ...
    Pentagram().scale(0.1).slate(0 pp 0.3).draw(Colour(0xFF006233), 2),
  )
}
...
{ val name = "Iraq"
  ...
  { topToBottom(Colour(0xFFce1126), White, Black) ++ Arr(
      PolyCurve(LineTail(-0.34 pp 0.2997), BezierTail(-0.3409 pp 0.3002, -0.3419 pp 0.301, -0.3423 pp 0.3015),
    ...
    PolyCurve(LineTail(-0.3304 pp 0.3084), BezierTail(-0.3313 pp 0.3096, -0.3325 pp 0.3141, -0.3321 pp 0.3153),
      ...
      PolyCurve(LineTail(-0.4429 pp 0.3117), BezierTail(-0.4432 pp 0.3095, -0.4391 pp 0.3041, -0.4372 pp 0.3031),
      ...
      PolyCurve(LineTail(-0.2945 pp 0.3121), BezierTail(-0.2903 pp 0.3099, -0.2871 pp 0.3068, -0.282 pp 0.3055),
      ...
      PolyCurve(LineTail(-0.3268 pp 0.2881), LineTail(-0.318 pp 0.2958), LineTail(-0.318 pp 0.2567), LineTail(-0.3117 pp 0.2567),
      ...
      PolyCurve(LineTail(-0.3478 pp 0.2571), BezierTail(-0.3466 pp 0.2553, -0.3425 pp 0.2553, -0.3427 pp 0.2583),
      ...
      PolyCurve(LineTail(-0.4041 pp 0.312), BezierTail(-0.3999 pp 0.3098, -0.3967 pp 0.3067, -0.3916 pp 0.3054),
...
{ val name = "India"
  ...
  val apply: GraphicElems =
  { 
    val spoke = PolyCurve(LineTail(-0.75 pp 0.3833), LineTail(-0.746 pp 0.4533), BezierTail(-0.746 pp 0.4533, -0.75 pp 0.4867, -0.75 pp 0.4867),
