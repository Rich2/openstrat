...
{ val name = "China"
  val ratio = 1.5
  val apply: Arr[GraphicElem] =Arr[GraphicElem](Rect.applyOld(1.5, 1).fill(Red),
    Rect.fromTL(0.75, 0.5, - 0.75 vv 0.5).fill(DarkBlue))
}
...
{ val name = "White"
  val ratio = 1.5
  val apply: Arr[GraphicElem] = Arr[GraphicElem](Rect.applyOld(1.5, 1).fill(White))
}
...
{ val name = "CommonShapesInFlags"
  ...
  {
    Arr[GraphicElem](
      Rect.applyOld(1.5, 1).fill(White),

      //off centre cross
      Rect.applyOld(ratio, 0.25).fill(Green),
      Rect.applyOld(0.25, 1).fill(Green).slate(-0.3 vv 0),
...
{ val name = "Eritrea"
  val ratio = 2.0
  val apply: Arr[GraphicElem] =
  { val blueRectangle = Rect.applyOld(2, 0.5, 0 vv 0.25).fill(Colour(0xFF4189dd))
    val greenRectangle =Rect.applyOld(2, 0.5, 0 vv -0.25).fill(Colour(0xFF12ad2b))
...
{ val name = "Papua New Guinea"
  val ratio = 4.0/3
  val apply: Arr[GraphicElem] =
  { val base = Rect.applyOld(ratio, 1).fill(Black)
