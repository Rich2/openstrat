...
{ def name: String
  ...
  def leftToRight(colours: Colour*): Arr[GraphicAffineElem] = colours.iMap((colour, i) => Rect.fromTL(ratio / colours.length, 1,
    -ratio / 2 vv + 0.5).slate(i * ratio / colours.length, 0).fillOld(colour))
         
  /** Equal height horizontal bands. width ratio should normally be greater than 1.0 */
  def topToBottom(colours: Colour*): Arr[GraphicAffineElem] = colours.iMap((colour, i) => Rect.fromTL(ratio,
     1.0 / colours.length, -ratio / 2 vv + 0.5).slate(0,
       - i.toDouble / colours.length).fillOld(colour))

  /** Equal height horizontal bands. width ratio should normally be greater than 1.0 */
  def topToBottomRepeat(numBands: Int, colours: Colour*): Arr[GraphicAffineElem] = iUntilMap(0, numBands){ i =>
    val r1 = Rect.fromTL(ratio, 1.0 / numBands, -ratio / 2 vv + 0.5)
    val r2 = r1.slate(0, - i.toDouble / numBands)
    r2.fillOld(colours(i %% colours.length))
  }
}
...
object PlainFlagMaker
{
  ...
  { override def name: String = colour.str + " Flag"
    ...
    override def apply(): Arr[GraphicElem] = Arr(rect.fillOld(colour))
  }
}
...
object TextFlagMaker
{
  ...
  { override def name: String = str + " Flag"
    override def ratio: Double = ratioIn
    override def apply(): Arr[GraphicElem] = Arr(rect.fillOld(colour), TextGraphic(str, 40))
  }
}
...
{ val name = "China"
  val ratio = 1.5
  val apply: Arr[GraphicElem] =Arr[GraphicElem](Rect.applyOld(1.5, 1).fillOld(Red),
    Rect.fromTL(0.75, 0.5, - 0.75 vv 0.5).fillOld(DarkBlue))
}
...
{ val name = "White"
  val ratio = 1.5
  val apply: Arr[GraphicElem] = Arr[GraphicElem](Rect.applyOld(1.5, 1).fillOld(White))
}
...
{ val name = "CommonShapesInFlags"
  ...
  {
    Arr[GraphicElem](
      Rect.applyOld(1.5, 1).fillOld(White),

      //off centre cross
      Rect.applyOld(ratio, 0.25).fillOld(Green),
      Rect.applyOld(0.25, 1).fillOld(Green).slate(-0.3 vv 0),

      Star5().scale(0.1).slate(-0.6 vv 0.3).fillOld(Magenta),

      Star7(0.382).scale(0.1).slate(-0.3 vv 0.3).fillOld(Red),
      ...
      //crescent
      Circle(0.225, -0.6, -0.3).fillOld(Red),
      Circle(0.2, -0.6, -0.3).slate(0.04 vv 0).fillOld(White),

      //composite star ()
      Star5().scale(0.15).slate(-0.3 vv 0).fillOld(Gold),
      Star5().scale(0.1).slate(-0.3 vv 0).fillOld(Magenta),
...
{ val name = "Iraq"
  ...
  { topToBottom(Colour(0xFFce1126), White, Black) ++ Arr[GraphicElem](
    ...
    PolyCurve(LineTail(-0.3304 vv 0.3084), BezierTail(-0.3313 vv 0.3096, -0.3325 vv 0.3141, -0.3321 vv 0.3153),
      ...
      Circle(0.0068, -0.5091, 0.2311).fillOld(Colour(0xFF007a3d)),
...
{ val name = "Eritrea"
  val ratio = 2.0
  val apply: Arr[GraphicElem] =
  { val blueRectangle = Rect.applyOld(2, 0.5, 0 vv 0.25).fillOld(Colour(0xFF4189dd))
    val greenRectangle =Rect.applyOld(2, 0.5, 0 vv -0.25).fillOld(Colour(0xFF12ad2b))
...
{ val name = "Papua New Guinea"
  val ratio = 4.0/3
  val apply: Arr[GraphicElem] =
  { val base = Rect.applyOld(ratio, 1).fillOld(Black)
