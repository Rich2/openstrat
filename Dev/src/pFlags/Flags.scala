...
{ def name: String
  ...
  def rect: PolygonGen = Rect(ratio)
  def parentStr: PolygonParent = Rect(ratio).parentElems(name + " flag", apply())
  def parent(evObj: Any = this): PolygonParent = Rect(ratio).parentElems(evObj, apply())
  ...
  def topToBottom(colours: Colour*): Arr[DisplayAffineElem] = colours.iMap((colour, i) => Rect.fromTL(ratio,
...
object PlainFlagMaker
{
  ...
  { override def name: String = colour.str + " Flag"
    ...
    override def apply(): Arr[DisplayElem] = Arr(rect.fillOld(colour))
  }
}
...
object TextFlagMaker
{
  ...
  { override def name: String = str + " Flag"
    override def ratio: Double = ratioIn
    override def apply(): Arr[DisplayElem] = Arr(rect.fillOld(colour), TextGraphic(str, 40))
  }
}
...
{ val name = "China"
  val ratio = 1.5
  val apply: Arr[DisplayElem] =Arr[DisplayElem](Rect(1.5, 1).fillOld(Red),
...
{ val name = "White"
  val ratio = 1.5
  val apply: Arr[DisplayElem] = Arr[DisplayElem](Rect(1.5, 1).fillOld(White))
}
...
{ val name = "CommonShapesInFlags"
  ...
  {
    Arr[DisplayElem](
      Rect(1.5, 1).fillOld(White),

      //off centre cross
      Rect(ratio, 0.25).fillOld(Green),
      Rect(0.25, 1).fillOld(Green).slate(-0.3 vv 0),
...
{ val name = "Eritrea"
  val ratio = 2.0
  val apply: Arr[DisplayElem] =
  { val blueRectangle = Rect(2, 0.5, 0 vv 0.25).fillOld(Colour(0xFF4189dd))
    val greenRectangle =Rect(2, 0.5, 0 vv -0.25).fillOld(Colour(0xFF12ad2b))
...
{ val name = "Papua New Guinea"
  val ratio = 4.0/3
  val apply: Arr[DisplayElem] =
  { val base = Rect(ratio, 1).fillOld(Black)
