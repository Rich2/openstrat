...
case class CanvasFx(canvFx: canvas.Canvas, theScene: Scene) extends CanvasTopLeft// with CanvSaver
{
  ...
  override def tlPolyFill(poly: PolygonTr, colour: Colour): Unit =
  { gc.setFill(toFxColor(colour))
    gc.fillPolygon(poly.elem1sArray, poly.elem2sArray, poly.length)
  }

  /** Needs mod */
  override protected[this] def tlPolyDraw(poly: PolygonTr, lineWidth: Double, colour: Colour): Unit =
  ...
  { val el = ef.shape
    ...
    gc.fillOval(el.xCen - el.majorRadius, el.yCen - el.minorRadius, el.majorRadius, el.minorRadius)
  }
  override def tlEllipseFill(ellipse: Ellipse, colour: Colour): Unit = 
  { gc.setFill(toFxColor(colour))
    gc.fillOval(ellipse.xCen - ellipse.majorRadius, ellipse.yCen - ellipse.minorRadius, ellipse.majorRadius, ellipse.minorRadius)
  }
