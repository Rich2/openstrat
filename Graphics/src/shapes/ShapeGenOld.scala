 ...
 *  a BezierSeg. */
class ShapeGenOld(val arrayUnsafe: Array[Double]) extends ArrDbl7s[CurveTail] with AffinePreserve
{ type ThisT = ShapeGenOld
  ...
  def ptInShape: Pt2 => Boolean = pt =>  dataMap[Pt2, PolygonGen](_.pEnd).ptInside(pt)
