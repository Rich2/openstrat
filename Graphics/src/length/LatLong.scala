...
final class LatLong private(val latMilliSecs: Double, val longMilliSecs: Double) extends LatLongBase with Show2Dbls
{
 ...
 // override def canEqual(other: Any): Boolean = other.isInstanceOf[LatLong]
  ...
  def fromFocusMetres(focus: LatLong): Metres3 = focus.subLongRadians(longRadians).toMetres3.xRotateRadians(-latRadians)
  ...
  { val r1: Metres3 = inp.subLongRadians(longRadians).toMetres3.xRotateRadians(-latRadians)
    r1.toXYIfZPositive
  }
