...
{ override def typeStr: String = "Pt2"
  ...
  @inline def toDist2(factor: Metres): Pt2M = Pt2M(x * factor, y * factor)
...
{ def apply(x: Double, y: Double): Pt2 = new Pt2(x, y)
  ...
  { def * (operand: Metres): Pt2M = Pt2M(thisPt.x * operand, thisPt.y * operand)
  }
  ...
  implicit val linePathBuildImplicit: Dbl2sLinePathBuilder[Pt2, LinePath] = new Dbl2sLinePathBuilder[Pt2, LinePath]
  { override type BuffT = Pt2Buff
