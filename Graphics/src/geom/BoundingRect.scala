...
{ def topLeft = Vec2(minX, maxY)
  ...
  def toPolygon: PolygonImp = PolygonImp(minX vv maxY, maxX vv maxY, maxX vv minY, minX vv minY)
  def ptInside(pt: Vec2): Boolean = maxX > pt.x & pt.x > minX & maxY > pt.y & pt.y > minY
  def toRectangle: PolygonImp = PolygonImp(minX vv maxY, maxX vv maxY, maxX vv minY, minX vv minY)
