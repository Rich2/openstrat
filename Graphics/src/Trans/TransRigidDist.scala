...
{ type ThisT <: TransRigidDister
  ...
  def ySlate(yDelta: Metres): ThisT = slate(Pt2M(0.metres, yDelta))

  /** Translate in 2 dimensional space. */
  def slate(xOffset: Metres, yOffset: Metres): ThisT = slate(Pt2M(xOffset, yOffset))
}
...
object TransRigidDist
{
  ...
  implicit def arrImplicit[A, AA <: SeqImut[A]](implicit build: ArrTBuilder[A, AA], ev: TransRigidDist[A]): TransRigidDist[AA] = new TransRigidDist[AA]
...
{ /** Translate 2 dimensional vectors along the X axis */
  def slateX(xOffset: Metres): T = ev.slate(value, Pt2M(xOffset, 0.metres))

  /** Translate 2 dimensional vectors along the Y axis */
  def slateY(yOffset: Metres): T = ev.slate(value, Pt2M(0.metres, yOffset))

  /** Translate in 2 dimensional space. */
  def slate(offset: Pt2M): T = ev.slate(value, offset)

  /** Translate in 2 dimensional space. */
  def slate(xOffset: Metres, yOffset: Metres): T = ev.slate(value, Pt2M(xOffset, yOffset))
}