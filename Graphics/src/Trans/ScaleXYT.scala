...
object ScaleXYT
{
  implicit def arrImplicit[A, AA <: ArrBase[A]](implicit build: ArrBuild[A, AA], ev: ScaleXYT[A]): ScaleXYT[AA] =
    (obj, xOperand: Double, yOperand) => obj.map(ev.scaleXYT(_, xOperand, yOperand))

  implicit def functorImplicit[A, F[_]](implicit evF: Functor[F], evA: ScaleXYT[A]): ScaleXYT[F[A]] =
    (obj, xOperand, yOperand) => evF.mapT(obj, evA.scaleXYT(_, xOperand, yOperand))

  implicit def arrayImplicit[A](implicit ct: ClassTag[A], ev: ScaleXYT[A]): ScaleXYT[Array[A]] =
    (obj, xOperand: Double, yOperand) => obj.map(ev.scaleXYT(_, xOperand, yOperand))
}

class XYScaleExtensions[T](val value: T, ev: ScaleXYT[T])
{
