...
{ type ThisT <: TransSimDisterUser
  ...
  override def slate(offset: Dist2): ThisT = newThis(geomMem.slate(offset).asInstanceOf[MemT])
  override def rotateRadians(radians: Double): ThisT = newThis(geomMem.rotateRadians(radians).asInstanceOf[MemT])
  override def mirrorYOffset(xOffset: Dist): ThisT = newThis(geomMem.mirrorYOffset(xOffset).asInstanceOf[MemT])
  override def mirrorXOffset(yOffset: Dist): ThisT = newThis(geomMem.mirrorXOffset(yOffset).asInstanceOf[MemT])
...
object TransSimDist
{
  ...
  { override def scale(obj: AA, operand: Double): AA = obj.map{ts => ev.scale(ts, operand)}
    ...
    override def mirrorYOffset(obj: AA, xOffset: Dist): AA = obj.map{ts => ev.mirrorYOffset(ts, xOffset) }
