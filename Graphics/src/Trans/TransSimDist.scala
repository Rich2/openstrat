...
{ type ThisT <: TransSimDisterUser
  ...
  override def slate(offset: Pt2M): ThisT = newThis(geomMem.slate(offset).asInstanceOf[MemT])
  override def rotateRadians(radians: Double): ThisT = newThis(geomMem.rotateRadians(radians).asInstanceOf[MemT])
  override def mirrorYOffset(xOffset: Metre): ThisT = newThis(geomMem.mirrorYOffset(xOffset).asInstanceOf[MemT])
  override def mirrorXOffset(yOffset: Metre): ThisT = newThis(geomMem.mirrorXOffset(yOffset).asInstanceOf[MemT])
...
object TransSimDist
{
  ...
  implicit def arrImplicit[A, AA <: ArrImut[A]](implicit build: ArrTBuilder[A, AA], ev: TransSimDist[A]): TransSimDist[AA] = new TransSimDist[AA]
