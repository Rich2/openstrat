...
//   def tR(x: Double, y: Double, width: Double, height: Double): Polygon = Polygon.xy(x - width, y, x, y, x, y - height, x - width, y -height)
//   def bR(x: Double, y: Double, width: Double, height: Double): Polygon = Polygon.xy(x - width, y + height, x, y + height, x, y, x - width, y)
   def fromBL(width: Double, height: Double, v: Vec2): Polygon = Polygon(v.x vv v.y + height,
         v.x + width vv v.y + height,
         v.x + width vv v.y,
         v.x vv v.y)
//   def bL(x: Double, y: Double, width: Double, height: Double): Polygon = Polygon.xy(x, y + height, x + width, y + height, x + width, y, x, y)
//
...
//   def fromBottomCentre(width: Double, height: Double, bottomCentre: Vec2 = Vec2Z): Polygon = Polygon.xy(
//         - width / 2, + height,
//         + width / 2, + height ,
//         + width / 2, 0,
//         - width / 2, 0)   
   ...
   def cross(width: Double, height: Double, barWidth: Double): List[Polygon] = List(apply(width, barWidth), apply(barWidth, height))
...
object Star5
{
   def apply(): Polygon =
   {
      ...
      (0 to 4).trav2ProdD2[Vec2, Vec2, Polygon](l2, (i, l) => l.rotate(-deg72 * i))
   }
