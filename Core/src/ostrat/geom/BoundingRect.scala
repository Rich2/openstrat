...
case class BoundingRect(minX: Double, maxX: Double, minY: Double, maxY: Double)
{
   ...
   def toVec2s: Vec2s = Vec2s(minX vv maxY, maxX vv maxY, maxX vv minY, minX vv minY)
   def ptInside(pt: Vec2): Boolean = maxX > pt.x & pt.x > minX & maxY > pt.y & pt.y > minY
   //def toRectangle: Vec2s = Vec2s(minX vv maxY, maxX vv maxY, maxX vv minY, minX vv minY)
}
