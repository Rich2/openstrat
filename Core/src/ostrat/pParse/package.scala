...
package object pParse
{
  ...
  def getStatements(input: String, inputSourceName: String): EMonList[Statement] = TokensGet(input, inputSourceName).flatMap(GetStatements(_))
  ...
  def stringToTokens(srcStr: String): EMon[List[Token]] = TokensGet(srcStr, "From string")  
}