...
{ override def apply(hc: HCen): RArr[A] = new RArr(outerArrayUnsafe(gridSys.layerArrayIndex(hc)))
  ...
  def copy: LayerHcRArr[A] = new LayerHcRArr[A](outerArrayUnsafe.clone, gridSys)
  ...
  def headsFlatMap[BB <: Arr[?]](f: (HCen, A) => BB)(implicit build: BuilderFlatArr[BB]): BB =
  { val buff = build.newBuff()
    gridSys.foreach { r =>
      val el:RArr[A] = apply(r)
  ...
  def headsHcFlatMap[ArrT <: Arr[?]](f: (A, HCen) => ArrT)(implicit build: BuilderFlatArr[ArrT]): ArrT =
  { val buff = build.newBuff()
    gridSys.foreach { hc =>
      if (noneEmptyTile(hc)) {
