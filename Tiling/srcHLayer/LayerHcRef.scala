...
{ /** Apply method returns a data element from this data layer for the given [[HCen]]. The appropriate index is found from the implicit [[HGridSys]]. There is
  ...
  def apply(hc: HCen)(using key: HexStruct): A = arrayUnsafe(key.layerArrayIndex(hc))
  ...
  def rc(r: Int, c: Int)(using key: HexStruct): A = arrayUnsafe(key.layerArrayIndex(r, c))

  def rc(key: HexStruct, r: Int, c: Int): A = arrayUnsafe(key.layerArrayIndex(r, c))

  def set(hc: HCen, value: A)(using gridSys: HexStruct): Unit = { arrayUnsafe(gridSys.layerArrayIndex(hc)) = value }
  def set(r: Int, c: Int, value: A)(using gridSys: HexStruct): Unit = { arrayUnsafe(gridSys.layerArrayIndex(r, c)) = value }
  ...
  def hcForeach[U](f: (HCen, A) => U)(using gSys: HexStruct): Unit = gSys.iForeach{ (i, hc) => f(hc, arrayUnsafe(i)); () }
   ...
   * signature. */
  def hcMap[B, BB <: Arr[B]](f: (HCen, A) => B)(using grid: HexStruct, build: BuilderArrMap[B, BB]): BB =
  ...
  def hcOptMap[B, BB <: Arr[B]](f: (A, HCen) => Option[B])(using grid: HexStruct, build: BuilderArrMap[B, BB]): BB =
   ...
   * B signature. */
  def hcFlatMap[BB <: Arr[?]](f: (HCen, A) => BB)(using grid: HexStruct, build: BuilderArrFlat[BB]): BB =
  { val buff = build.newBuff()
    grid.iForeach{ (i, hc) =>
   ...
   * signature follows the foreach based convention of putting the collection element 2nd or last as seen for example in fold methods' (accumulator, element) =>
   * B signature. */
  def hcOptFlatMap[BB <: Arr[?]](f: (HCen, A) => Option[BB])(using gridSys: HexStruct, build: BuilderArrFlat[BB]): BB =
  { val buff = build.newBuff()
  ...
  def projHCenFlatMap[BB <: Arr[?]](f: (HCen, A) => BB)(using key: HexStruct, proj: HSysProjection, build: BuilderArrFlat[BB]): BB =
    proj.hCenFlatMap{ hc => f(hc, apply(hc)(using key)) }
}
...
{
  def apply[A <: AnyRef](row: Int, elems: A*)(using ct: ClassTag[A]): LayerHcRefRow[A] =
  ...
  implicit def unshowEv[A <: AnyRef](implicit evA: Unshow[A], ct: ClassTag[A]): Unshow1Repeat[Int, A, LayerHcRefRow[A]] =
    Unshow1Repeat[Int, A, LayerHcRefRow[A]]("HRow", "row", "values",  (r, seq) => new LayerHcRefRow[A](r, seq.toArray))(using Unshow.intSubset(_.isEven), evA)
