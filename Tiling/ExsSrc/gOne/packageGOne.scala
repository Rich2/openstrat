 ...
 * move to a tile already containing a player or that one more than one player is attempting to move to fails.his is the package for the second
 * example of simple simultaneous turn tile based games. It differs from the first in that it is on a square grid
...
package object gOne
{
  ...
  one hex tile step. Any move to a tile already containing a player or that one more than one player is attempting to move to fails.This is the
   package for the second example of simple simultaneous turn tile based games. It differs from the first in that it is on a square grid
    and adjacent moves take priority over diagonal tile steps.."""
}