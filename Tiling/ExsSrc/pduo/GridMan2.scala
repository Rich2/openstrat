...
class GridMan2(val sys: HGridsDuo, val grid: HGridReg) extends HGridMan
{
  ...
  override def outSteps(r: Int, c: Int): HStepCenArr = (r, c) match {
    case (r, c) if r == sys.maxCenR & c == sys.minC2 => HStepCenArr((HexLt, r, c + sys.grid2OffsetC - 4), (HexDL, r - 2, c + sys.grid2OffsetC - 2))
    case (r, c) if r == sys.maxCenR & c == sys.minC2 + 2 => HStepCenArr((HexLt, r, c + sys.grid2OffsetC - 4))

    case (r, c) if r == sys.minCenR & c == sys.minC2 => HStepCenArr((HStepUL, r + 2, c + sys.grid2OffsetC - 2), (HexLt, r, c + sys.grid2OffsetC - 4))
    case (r, c) if r == sys.minCenR & c == sys.minC2 + 2 => HStepCenArr((HexLt, r, c + sys.grid2OffsetC - 4))

    case (r, c) if c == sys.minC2 =>
      HStepCenArr((HStepUL, r + 2, c + sys.grid2OffsetC - 2), (HexLt, r, c + sys.grid2OffsetC - 4), (HexDL, r - 2, c + sys.grid2OffsetC - 2))
    case (r, c) if c == sys.minC2 + 2 => HStepCenArr((HexLt, r, c + sys.grid2OffsetC - 4))
