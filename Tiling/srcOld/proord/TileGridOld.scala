...
import geom._, reflect.ClassTag, Colour._, proord._
...
trait TileGridOld
{
  ...
  final def map[A, ArrT <: SeqImut[A]](f: Roord => A)(implicit build: ArrBuilder[A, ArrT]): ArrT =
  ...
  final def iMap[A, ArrT <: SeqImut[A]](f: (Roord, Int) => A)(implicit build: ArrBuilder[A, ArrT]): ArrT =
  { val res = build.newArr(numOfTiles)
    var i = 0
    foreach{ roord =>
  ...
  final def flatMap[ArrT <: SeqImut[_]](f: Roord => ArrT)(implicit build: ArrFlatBuilder[ArrT]): ArrT =
  ...
  final def flatMapNoDupicates[A, ArrT <: SeqImut[A]](f: Roord => ArrT)(implicit build: ArrBuilder[A, ArrT]): ArrT =
  { val buff = build.newBuff(numOfTiles)
    foreach { roord =>
      val newVals = f(roord)
  ...
  final def mapRPolygons[A, ArrT <: SeqImut[A]](f: (Roord, PolygonGen) => A)(implicit build: ArrBuilder[A, ArrT]): ArrT =
    map { roord =>
  ...
  def setTile[A](roord: Roord, value: A)(implicit arr: SeqImut[A]): Unit = arr.unsafeSetElem(arrIndex(roord), value)
  ...
  def sidesMap[A, ArrT <: SeqImut[A]](f: Roord => A)(implicit build: ArrBuilder[A, ArrT]) =
  ...
  /** Maps from each sides Roord to an ArrBase of A. */
  def sidesIMap[A, ArrT <: SeqImut[A]](f: (Roord, Int) => A)(implicit build: ArrBuilder[A, ArrT]) =
  { val res = build.newArr(numOfSides)
    var count = 0
  ...
  def vertsMap[A, ArrT <: SeqImut[A]](f: Roord => A)(implicit build: ArrBuilder[A, ArrT]) =
  ...
  def vertsIMap[A, ArrT <: SeqImut[A]](f: (Roord, Int) => A)(implicit build: ArrBuilder[A, ArrT]) =
  { val res = build.newArr(numOfVerts)
    var count = 0
