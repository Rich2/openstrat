...
abstract class EArea2(val name: String, val cen: LatLong, val terr: WTile) extends GeographicSymbolKey
{
  ...
  /** Returns a pair of this [[EArea2]] and the [[PolygonM]] from the given focus and orientation. The polygonM only has points form the side side of
   *  the earth that faces the focus.  */
  def withPolygonM(focus: LatLong, northUp: Boolean): (EArea2, PolygonM) =
  { val p3s0: PolygonM3 = polygonLL.toMetres3
    ...
    val p3s3: PolygonM = p3s2.earthZPosXYModify
    (this, p3s3)
  }
}
