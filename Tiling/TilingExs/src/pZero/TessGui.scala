...
{ val d1 = 80.0
  ...
  val lc1 = -4
  val rc1 = 4
  val g1 = HGridRect.minMax(br1, tr1, lc1, rc1)
  val f1: HCoord => Pt2 = hc => hc.toPt2Reg.slateFrom(g1.cenVec).scale(d1)
  val s1 = g1.sepsMap(hs => hs.lineSegHC.map(f1).draw())
  val g2 = HGridRect.minMax(br1 * 2, tr1 * 2 + 2, lc1 * 2, rc1 * 2 + 4)
  val g3 = HGridRect.minMax(br1 * 4, tr1 * 4 + 2, lc1 * 4, rc1 * 4 + 4)
  val f2: HCoord => Pt2 = hc => hc.toPt2Reg.slateFrom(g1.cenVec).slateFrom(2, 3.sqrt).scale(d2)
  val s2 = g2.sepsMap(hs => hs.lineSegHC.map(f2).draw(lineColour = Red))
  val t2 = g2.map(hc => f2(hc).textAt(hc.rcStr, 12, Red))
  val f3: HCoord => Pt2 = hc => hc.toPt2Reg.slateFrom(g1.cenVec).slateFrom(0, 0).scale(d2 / 2)
