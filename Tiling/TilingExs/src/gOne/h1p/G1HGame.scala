...
{ implicit def gridSys: HGridSys = scen.gridSys
  ...
  /** Resolves turn. Takes an [[HCenStepArr]]. The directives are passed in as relative moves. This is in accordance with the principle in more complex games
   * that the entity issuing the command may not know its real location. */
  def endTurn(directives: HCenStepArr): G1HScen =
  { val intensions: HCenOptStepLayer = HCenOptStepLayer()(using scen.gridSys)
