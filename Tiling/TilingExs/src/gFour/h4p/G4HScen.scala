...
{  def teamSet: RArr[Team]
  ...
  def endTurn(orderList: HStepPathPairArr[LunitState]): G4HScen = ???
  /*{
    ...
    G4HScen(turn + 1, gridSys, terrs, oPlayersNew)
  }*/
}

object G4HScen
{
  def apply(turnNum: Int, gridIn: HGrid, terrsIn: LayerHcRefSys[Terr], unitsIn: LayerHcRArr[LunitState]): G4HScen = new G4HScen(turnNum)
  {
    /** tile terrain. */
   override def terrs: LayerHcRefSys[Terr] = terrsIn

    override def lunits: LayerHcRArr[LunitState] = unitsIn
