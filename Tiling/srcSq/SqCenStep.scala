...
{ /** The Starting square centre coordinate. */
  ...
  /** Returns the destination [[SqCen]] if one exists within the [[SqGridSys]]. */
  def lineSegSqC(using gridSys: SqGridSys): Option[LSegSC] = gridSys.stepEndFind(this).map(LSegSC(startSC, _))

  def projLineSeg(implicit proj: SqSysProjection): Option[LSeg2] =
  { val lhc: Option[LSegSC] = lineSegSqC(using proj.gChild)
...
object SqCenStepArr extends CompanionSlInt3[SqCenStep, SqCenStepArr]
{ override def fromArray(array: Array[Int]): SqCenStepArr = new SqCenStepArr(array)

  implicit val flatBuildEv: BuilderFlatArrInt3[SqCenStepArr] = new BuilderFlatArrInt3[SqCenStepArr]{
    override type BuffT = SqCenStepBuff
    override def fromIntArray(array: Array[Int]): SqCenStepArr = new SqCenStepArr(array)
...
{ override type ThisT = SqCenStepBuff
  ...
  override def elemFromInts(i1: Int, i2: Int, i3: Int): SqCenStep = new SqCenStep(i1, i2, i3)
}
...
{ override type ThisT = SqCenStepPairArr[A2]
  ...
  override def a1FromInts(int1: Int, int2: Int, int3: Int): SqCenStep = new SqCenStep(int1, int2, int3)
...
object SqCenStepPairArr extends CompanionArrPairInt3[SqCenStep]
{
