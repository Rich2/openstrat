...
{ def startR: Int = arrayUnsafe(0)
  ...
  def segHCsMap[B, ArrB <: Arr[B]](f: LineSegHC => B)(implicit build: BuilderMapArr[B, ArrB], grider: HGridSys): ArrB =
...
/** An [[PairFinalA1Elem]] where the first element is an [[HStepPath]], a path consisting of a starting [[HCen]] and a sequence of [[HStep]]s. */
