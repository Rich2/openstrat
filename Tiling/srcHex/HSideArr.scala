...
class HSideArr(val unsafeArray: Array[Int]) extends AnyVal with ArrInt2[HSep]
{ type ThisT = HSideArr
...
/** Companion object for [[HSideArr]] trait efficient array[Int] based collection for [[HSep]]s hex grid centre coordinates, contains factory apply and
 *  uninitialised methods. */
object HSideArr extends CompanionSeqLikeInt2[HSep, HSideArr]
{ override def fromArray(array: Array[Int]): HSideArr = new HSideArr(array)
  ...
  implicit lazy val showEv: ShowSequ[HSep, HSideArr] = ShowSequ[HSep, HSideArr]()

  /** Implicit [[Unshow]] type class instance / evidence for [[HSideArr]]. */
  implicit lazy val unshowEv: UnshowSeq[HSep, HSideArr] = UnshowSeq[HSep, HSideArr]()
}

class HSideBuff(val unsafeBuffer: ArrayBuffer[Int] = BufferInt()) extends AnyVal with BuffInt2[HSep]
{ type ArrT = HSideArr
...
class HSidePairArr[A2](val a1ArrayInt: Array[Int], val a2Array: Array[A2]) extends ArrPairInt2[HSep, HSideArr, A2, HSidePair[A2]]
...
{
  def unapply[A](inp: HSidePairArr[A]): Option[(HSep, A)] = inp match{
...
class HSidePairBuff[B2](val b1IntBuffer: ArrayBuffer[Int], val b2Buffer: ArrayBuffer[B2]) extends BuffPairInt2[HSep, B2, HSidePair[B2]]
...
class HSidePairArrMapBuilder[B2](implicit ct: ClassTag[B2]) extends BuilderArrPairInt2Map[HSep, HSideArr, B2, HSidePair[B2], HSidePairArr[B2]]
{ override type BuffT = HSidePairBuff[B2]
  ...
  override def b1ArrBuilder: BuilderArrMap[HSep, HSideArr] = HSep.arrMapBuilderEv
