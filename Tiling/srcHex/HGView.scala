...
class HGView(val r: Int, val c: Int, val pixelsPerC: Double, orientUp: Boolean) extends Tell3[HCoord, Double, Boolean]
{ def hCoord: HCoord = HCoord(r, c)
  ...
  override def name3: String = "isUp"
  override def tell3: Boolean = orientUp
...
{ def apply(r: Int, c: Int, cPScale: Double = 50, orientUp: Boolean = true): HGView = new HGView(r, c, cPScale, orientUp)
  ...
  implicit val unshowEv: Unshow3[HCoord, Double, Boolean, HGView] =
    Unshow3[HCoord, Double, Boolean, HGView]("HGView", "hCoord", "cPScale", "isUp", apply, Some(true))
}