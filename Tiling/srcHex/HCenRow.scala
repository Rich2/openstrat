...
final class HCenRow(val r: Int, val c: Int, val tNum: Int)
{
  def cLen: Int = tNum / 4
  ...
  { val res = new Array[Int]((tNum * 4 + 2) * 2)
    ...
    iToForeach(tNum, 2, - 1) { i =>
      res.setIndex2(tNum * 4 - i * 2 - 1, r - 1, c + i * 4 - 2)
      res.setIndex2(tNum * 4 - i * 2,     r - 1, c + i * 4 - 4)
    }
    res.setIndex2(tNum * 4 - 3, r - 1, c + 2)
    res.setIndex2(tNum * 4 - 2, r - 1, c)
    res.setIndex2(tNum * 4 - 1, r - 1, c - 2)
    res.setIndex2(tNum * 4,     r + 1, c - 2)
    res.setIndex2(tNum * 4 + 1, r + 1, c)
    res
  }
}
