...
class HCenArrOpt[A <: AnyRef](val unsafeArr: Array[A]) extends AnyVal
{
  ...
  def cMap[B, ArrT <: SeqImut[B]](fNone: => HCen => B)(fSome: (A, HCen) => B)(implicit grid: HGrid, build: SeqBuild[B, ArrT]): ArrT =
  {
  ...
  def map[B, ArrT <: SeqImut[B]](noneValue: => B)(f: A => B)(implicit grid: HGrid, build: SeqBuild[B, ArrT]): ArrT =
  {
    val buff = build.newBuff()
    grid.foreach { r =>
  ...
  def mapSomes[B, ArrT <: SeqImut[B]](f: A => B)(implicit build: SeqBuild[B, ArrT]): ArrT =
  {
    val buff = build.newBuff()
    unsafeArr.foreach { a =>
      if(a != null)
  ...
  def cMapSomes[B, ArrT <: SeqImut[B]](f: (A, HCen) => B)(implicit grid: HGrid, build: SeqBuild[B, ArrT]): ArrT =
  {
    val buff = build.newBuff()
    grid.foreach { r =>
  ...
  def cMapNones[B, ArrT <: SeqImut[B]](f: HCen => B)(implicit grid: HGrid, build: SeqBuild[B, ArrT]): ArrT =
  {
    val buff = build.newBuff()
    grid.foreach { r =>
      val a: A = unsafeArr(grid.arrIndex(r))
  ...
  def cFlatMapSomes[ArrT <: SeqImut[_]](f: (A, HCen) => ArrT)(implicit grid: HGrid, build: SeqFlatBuild[ArrT]): ArrT =
  {
    val buff = build.newBuff()
    grid.foreach { hc =>
      val a = unsafeArr(grid.arrIndex(hc))
