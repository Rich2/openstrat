...
{ /** The number of tile centre rows where r %% 4 == 0.  */
  ...
  final def map[B, ArrB <: SeqImut[B]](f: HCen => B)(implicit build: SeqBuild[B, ArrB]): ArrB =
  ...
  final def flatMap[ArrT <: SeqImut[_]](f: HCen => ArrT)(implicit build: SeqFlatBuild[ArrT]): ArrT =
   ...
   *  @group SidesGroup */
  final def sidesMap[B, ArrT <: SeqImut[B]](f: HSide => B)(implicit build: SeqBuild[B, ArrT]): ArrT =
  {
  ...
  /** maps over each Hex Side's coordinate [[HSide]] in the given Row.
   *  @group SidesGroup */
  final def sidesFlatMap[ArrT <: SeqImut[_]](f: HSide => ArrT)(implicit build: SeqFlatBuild[ArrT]): ArrT =
  {
