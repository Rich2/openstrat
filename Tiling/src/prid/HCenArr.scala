...
class HCenArr[A <: AnyRef](val unsafeArr: Array[A])
{
  ...
  def map[B, BB <: SeqImut[B]](f: A => B)(implicit build: SeqBuild[B, BB]): BB =
  ...
  def mapHC[B, BB <: SeqImut[B]](f: (A, HCen) => B)(implicit grid: HGrid, build: SeqBuild[B, BB]): BB =
  { val res = build.newArr(length)
    grid.iForeach{ (hc, i) =>
   ...
   *  tile data values. */
  def sideMap[B, BB <: SeqImut[B]](f1: (HSide, A) => B, f2: (HSide, A, A) => B)(implicit grid: HGrid, build: SeqBuild[B, BB]): BB =
   ...
   *  [[HSide]] and the single adjacent hex tile data value and one for the inner sides of the grid that takes the [[HSide]] and the two adjacent hex
   *  tile data values. */
  def sideFlatMap[BB <: SeqImut[_]](f1: (HSide, A) => BB, f2: (HSide, A, A) => BB)(implicit grid: HGrid, build: SeqFlatBuild[BB]): BB =
