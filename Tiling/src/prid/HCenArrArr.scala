...
class HCenArrArr[A](val unsafeArray: Array[Array[A]])
{
  ...
  def gridHeadsMap[B, BB <: SeqImut[B]](f: (HCen, A) => B)(implicit grid: HGrid, build: ArrTBuilder[B, BB]): BB =
  {
  ...
  /** flatMaps over the the first element of each tile's data Array. Ignores empty arrays and subsequent elements. */
  def gridHeadsFlatMap[BB <: SeqImut[_]](f: (HCen, A) => BB)(implicit grid: HGrid, build: ArrTFlatBuilder[BB]): BB =
  {
    val buff = build.newBuff()
    grid.foreach { r =>
      val el:Arr[A] = apply(r)
