...
class SqCenArrOpt[A <: AnyRef](val unsafeArr: Array[A]) extends AnyVal
{
  ...
  def cMapSomes[B, ArrT <: SeqImut[B]](f: (A, SqCen) => B)(implicit grid: SqGrid, build: SeqBuild[B, ArrT]): ArrT =
  {
   ...
   *  an immutable Array based collection of type ArrT, the second type parameter. */
  def cMapNones[B, ArrT <: SeqImut[B]](f: SqCen => B)(implicit grid: SqGrid, build: SeqBuild[B, ArrT]): ArrT =
  {
    val buff = build.newBuff()
    grid.foreach { r =>
