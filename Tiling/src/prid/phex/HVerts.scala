...
{ def fromArray(array: Array[Int]): HVerts = new HVerts(array)
  ...
  { override def fromArray(value: Array[Int]): HVerts = new HVerts(value)
    ...
    override def showDecT(obj: HVerts, way: ShowStyle, maxPlaces: Int, minPlaces: Int): String = ???
  }
...
class HVertBuff(val unsafeBuffer: Buff[Int] = buffInt()) extends AnyVal with BuffInt2s[HVert]
{ type ArrT = HVerts
