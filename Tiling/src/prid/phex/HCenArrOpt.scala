...
class HCenArrOpt[A <: AnyRef](val unsafeArr: Array[A]) extends AnyVal with TileArrOpt[A]
{
  ...
  def unsafeSetSome(r: Int, c: Int, value: A)(implicit grid: HGridBased): Unit = unsafeArr(grid.arrIndex(r, c)) = value

  /** Sets the Some value of the hex tile data at the specified [[HCen]] coordinate. This is an imperative mutating operation. */
  def unsafeSetSome(hc: HCen, value: A)(implicit grid: HGridBased): Unit = unsafeArr(grid.arrIndex(hc)) = value
  ...
  def unsafeSetSomes(triples: (Int, Int, A)*)(implicit grid: HGridBased): Unit = triples.foreach(t => unsafeArr(grid.arrIndex(t._1, t._2)) = t._3)
  ...
  def setSome(hc: HCen, value: A)(implicit grid: HGridBased): HCenArrOpt[A] =
  ...
  def setNone(hc: HCen)(implicit grid: HGridBased): HCenArrOpt[A] =
  { val newArr = unsafeArr.clone()
   ...
   *  None. */
  def unsafeMove(hc1: HCen, hc2: HCen)(implicit grid: HGridBased): Unit =
   ...
   *  the None values. */
  def hcSomesForeach(f: (HCen, A) => Unit)(implicit grid: HGridBased): Unit = grid.foreach { hc =>
  ...
  def mapHCen[B, ArrT <: SeqImut[B]](fNone: => HCen => B)(fSome: (HCen, A) => B)(implicit grid: HGridBased, build: ArrBuilder[B, ArrT]): ArrT =
  ...
  def apply(hc: HCen)(implicit grid: HGridBased): Option[A] =
  ...
  /** Indexes in to this [[HCenArrOpt]] using the tile centre coordinate, either passed as an [[HCen]] or as row and column [[Int values]]. */
  def apply(r: Int, c: Int)(implicit grid: HGridBased): Option[A] = {
  ...
  def unSafeApply(hc: HCen)(implicit grid: HGridBased): A = unsafeArr(grid.arrIndex(hc))

  /** The tile is a None at the given hex grid centre coordinate [[HCen]]. */
  def tileNone(hc: HCen)(implicit grid: HGridBased): Boolean = unsafeArr(grid.arrIndex(hc)) == null
  ...
  def hcSomesMap[B, ArrB <: SeqImut[B]](f: (HCen, A) => B)(implicit grid: HGridBased, build: ArrBuilder[B, ArrB]): ArrB =
  { val buff = build.newBuff()
  ...
  def some2sMap[B <: AnyRef, C, ArrC <: SeqImut[C]](optArrB: HCenArrOpt[B])(f: (A, B) => C)(implicit grid: HGridBased, build: ArrBuilder[C, ArrC]): ArrC =
  { val buff = build.newBuff()
  ...
  def hcSome2sMap[B <: AnyRef, C, ArrC <: SeqImut[C]](optArrB: HCenArrOpt[B])(f: (HCen, A, B) => C)(implicit grid: HGridBased, build: ArrBuilder[C, ArrC]):
    ArrC =
  { val buff = build.newBuff()
  ...
  def hcNonesMap[B, ArrB <: SeqImut[B]](f: HCen => B)(implicit grid: HGridBased, build: ArrBuilder[B, ArrB]): ArrB =
  { val buff = build.newBuff()
  ...
  def hcSomesFlatMap[ArrT <: SeqImut[_]](f: (HCen, A) => ArrT)(implicit grid: HGridBased, build: ArrFlatBuilder[ArrT]): ArrT =
  {
    val buff = build.newBuff()
    grid.foreach { hc =>
      val a = unsafeArr(grid.arrIndex(hc))
  ...
  def keyMap(implicit grid: HGridBased): Map[A, HCen] =
  { val build = Map.newBuilder[A, HCen]
    hcSomesForeach((p, hc) => build.addOne(hc, p))
    build.result
  }

  def find(value: A)(implicit grid: HGridBased): Option[HCen] =
