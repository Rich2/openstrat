...
{ def grid: GridT
  ...
  *   object's members are imported */
  ...
  def ifScaleCObj(ifScale: Double, cObj: DisplayElem *): DisplayElems = if (tScale > ifScale) cObj.toArr else Arr()

  def ifScaleIfCObj(ifScale: Double, b: Boolean, cObjs: DisplayElem *): DisplayElems = if (tScale > ifScale && b) cObjs.toArr else Arr()
...
{ def side: SideT    
  ...
  def vertDispLine: Sline = coodsLine.toLine2(coodToDispVec2)
