...
trait TileGridOld
{
  ...
  final def map[A, ArrT <: SeqImut[A]](f: Roord => A)(implicit build: ArrTBuilder[A, ArrT]): ArrT =
  ...
  final def iMap[A, ArrT <: SeqImut[A]](f: (Roord, Int) => A)(implicit build: ArrTBuilder[A, ArrT]): ArrT =
  { val res = build.newArr(numOfTiles)
    var i = 0
    foreach{ roord =>
  ...
  final def flatMap[ArrT <: SeqImut[_]](f: Roord => ArrT)(implicit build: ArrTFlatBuilder[ArrT]): ArrT =
  ...
  final def flatMapNoDupicates[A, ArrT <: SeqImut[A]](f: Roord => ArrT)(implicit build: ArrTBuilder[A, ArrT]): ArrT =
  { val buff = build.newBuff(numOfTiles)
    foreach { roord =>
      val newVals = f(roord)
  ...
  final def mapRPolygons[A, ArrT <: SeqImut[A]](f: (Roord, PolygonGen) => A)(implicit build: ArrTBuilder[A, ArrT]): ArrT =
    map { roord =>
  ...
  def sidesMap[A, ArrT <: SeqImut[A]](f: Roord => A)(implicit build: ArrTBuilder[A, ArrT]) =
  ...
  /** Maps from each sides Roord to an ArrBase of A. */
  def sidesIMap[A, ArrT <: SeqImut[A]](f: (Roord, Int) => A)(implicit build: ArrTBuilder[A, ArrT]) =
  { val res = build.newArr(numOfSides)
    var count = 0
  ...
  def vertsMap[A, ArrT <: SeqImut[A]](f: Roord => A)(implicit build: ArrTBuilder[A, ArrT]) =
  ...
  def vertsIMap[A, ArrT <: SeqImut[A]](f: (Roord, Int) => A)(implicit build: ArrTBuilder[A, ArrT]) =
  { val res = build.newArr(numOfVerts)
    var count = 0
