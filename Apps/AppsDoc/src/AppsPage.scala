...
{ override def titleStr: String = "Applications Module"
  ...
  def stratList: OlSection = OlSection(H2Html("Strategy Games using tiled world maps."),
    LiHtml.linkAndText(egameDir / "dicelessapp.html", "DiceLess", "A simple simultaneous turn multi player game set in Europe in 1900. As the name suggests"
      -- "no random element."),
    LiHtml.linkAndText(egameDir / "periculoapp.html", "Periculo Fundatuso", "A simple consecutive turn, world map game that has some grounding in earth" --
      "geography."),
    LiHtml.linkAndText(egameDir / "ww2app.html", "World War II Game", "Using 460km scale."),

    LiHtml.linkAndText(egameDir / "bc305app.html", "BC305", """A grand strategy game with a start point of 305BC, using 220km scale. This is the game that most
    | interests me.""".stripMargin),

    LiHtml.linkAndText(egameDir / "ww1app.html", "World War I Game", "A 120km hex game."),
    LiHtml.linkAndText(egameDir / "sorsapp.html", "Sors Imperiorum", "A game where empires appear at set times according to history. Uses 220km hex scale"),

    LiHtml.linkAndText(egameDir / "discovapp.html", "Age of Discovery",
      "A grand strategy game with a start point of 1492. It also uses an 160km scale world map."),

    LiHtml.linkAndText(egameDir / "indrevapp.html", "IndRev", "A grand strategy game with a start point of 1783. It also uses a 220km scale worldmap. This" --
      "is the second game that most interests me."),
  )

  def otherTiled: OlSection = OlSection(H2Html("Other Tiled Map Applications."),
    LiHtml.linkAndText(otDir / "unitlocapp.html", "Unit Locator", "Locates military units and gives information for a given date and time."),
    LiHtml.linkAndText(otDir / "zugapp.html", "Zug Fuhrer", "A Tactical strategy game with a 20 metre hex scale."),
    LiHtml.linkAndText(otDir / "dungeonapp.html", "Dungeon Game", "A Tactical strategy game on square tiles with a 0.5 metre tile scale."),
    LiHtml.linkAndText(otDir / "civriseapp.html", "Civ Rise", "A 4X strategy game using hexs. Its main use so far has been to develop a generalised" --
  ...
  def otherApps: OlSection = OlSection(H2Html("Other Apps"),
    ...
    LiHtml.linkAndText(otDir / "planetsapp.html", "Planets",
    ...
    LiHtml.linkAndText(otDir / "flagsapp.html", "Flags",
      "Just some flags using the graphics module. Thanks to Rod and Stephen who did most of the work on this."),

    LiHtml.linkAndText(otDir / "chessapp.html", "Chess", "Not completed."))
}