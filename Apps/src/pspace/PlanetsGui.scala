...
import geom.*, pgui.*, Colour.*, java.time.*
...
{ val margin = 35
  ...
  def toCanv(mapPoint: PtM2): Pt2 = mapPoint.slateFrom(mapFocus).mapGeom2(scale)
  ...
  implicit class PrimaryBodyExtensions(thisBody: SSPrimaryBody)
  {
    ...
    def paint(elapsed: Integer): CircleFill = Circle(0.3 * dispSize, toCanv(thisBody.posn(elapsed))).fill(thisBody.colour)
