...
{ val atoms: RArr[Atom]
  ...
  def lines: LineSegLen2ArrDraw = bondSegs.draw()
...
{ val aH1 = HAtom()
  val aO1 = OAtom()
  val aH2 = HAtom()
  val bondsAngle: AngleVec = 104.52.degsVec
  val bondLen: Picometres = 95.72.picometres
  private val bondRem: AngleVec = (180.degsVec - bondsAngle) / 2
  override val atoms: RArr[Atom] = RArr(aH1, aO1, aH2)
  val a01Posn: PtPm2 = PtPm2.origin
  override val atomPosns: PtPm2Arr = PtPm2Arr(a01Posn.ptAtAngle(Degs(-180) + bondRem, bondLen), a01Posn, a01Posn.ptAtAngle(0.degs - bondRem, bondLen))
