...
{ val scale = 1.picometres// 0.01.angstroms
  val output: GraphicElems = CO2Mc.lines.map(_.mapGeom2(scale)).slateX(-400) ++ CO2Mc.circles.map(_.mapGeom2(scale)).slateX(-400) ++
    Water.circles.map(_.mapGeom2(scale)).slateX(400)
