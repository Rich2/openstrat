...
class Ray private(x0: Double, y0: Double, angleSecs: Double) extends LineLike
{
  ...
  override def slateFrom(operand: VecPt2): Ray = ???
  override def slateFrom(xOperand: Double, yOperand: Double): Ray = Ray.v0Secs(p0.slateFrom(xOperand, yOperand), angleSecs)
  ...
  override def mirror(lineLike: LineLike): Ray = ???
