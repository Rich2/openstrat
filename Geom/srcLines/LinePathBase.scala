...
{ type ThisT <: LinePathBase[VT]
  ...
  def map[B <: ValueNElem, BB <: LinePathBase[B]](f: VT => B)(implicit build: BuilderLinePathLikeMap[B, BB]): BB =
...
trait LinePathValueN[VT <: ValueNElem] extends Any, LinePathBase[VT], SeqSpecValueN[VT]
...
trait BuilderLinePathLikeMap[B, BB <: LinePathBase[B]] extends BuilderSeqLikeMap[B, BB]

/** Trait for creating the line path builder instances for the [[BuilderLinePathLikeMap]] type class, for classes / traits you control, should go in the companion
 * object of B. The first type parameter is called B, because to corresponds to the B in ```map(f: A => B): ArrB``` function. */
trait BuilderLinePathValueN[B <: ValueNElem, BB <: LinePathBase[B]] extends BuilderLinePathLikeMap[B, BB], BuilderSeqLike[BB]

trait BuilderLinePathLikeFlat[VT, +BB <: LinePathBase[VT]] extends BuilderSeqLikeFlat[BB @uncheckedVariance]
