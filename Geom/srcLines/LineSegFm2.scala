...
/** A 2-dimensional line segment measured in [[Femtometres]], equivalent of the [[LSeg]] class. A straight line between two points on a 2-dimensional flat
 *  surface. */
...
{ def xStart: Femtometres = Femtometres(xStartFemtometresNum)
  ...
  override def mapGeom2(operand: Length): LSeg = LSeg(xStartFemtometresNum / operand.femtometresNum, yStartFemtometresNum / operand.femtometresNum,
...
{ override def fromArray(array: Array[Double]): LineSegFm2Arr = new LineSegFm2Arr(array)
  ...
  implicit val flatBuildEv: BuilderFlatArr[LineSegFm2Arr] = new BuilderFlatArrDbl4[LineSegFm2Arr]
  { type BuffT = LineSegFm2Buff
    override def fromDblArray(array: Array[Double]): LineSegFm2Arr = new LineSegFm2Arr(array)
