...
{ type ThisT <: LinePathLike[VT]
  ...
  /** Reverses this line path, appends the operand line path and then closes to a polygon. The < character between the + characters indicates that it this line
   * path that is reversed. */
