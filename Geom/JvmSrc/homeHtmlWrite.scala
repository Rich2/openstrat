...
{ val home: String = System.getProperty("user.home")
  utiljvm.fileWrite(home / dir / title + ".html", HtmlPage.titleOnly(title, bodyContent).out)
}
...
{ val home: String = System.getProperty("user.home")
  utiljvm.fileWrite(home / "opensett" / title + ".html", HtmlPage.titleOnly(title, bodyContent).out)
}
...
{
  def writeFile(dirPathStr: String): ErrBi[IOExc, HtmlFileWritten] = fileWrite(dirPathStr / thisPage.fileName, thisPage.out).map(_.html)
}

implicit class CssRulesHolderExtensions(thisRules: CssRulesHolder)
{
  def writeFile(dirPathStr: String, fileNameStem: String): ErrBi[IOExc, CssFileWritten] = fileWrite(dirPathStr / fileNameStem + ".css", thisRules.out).map(_.css)
}