...
final class MyDbl2Arr(val arrayUnsafe: Array[Double]) extends AnyVal with ArrDbl2[MyDbl2Elem]
{ type ThisT = MyDbl2Arr
  def typeStr = "Mines"
  def fromArray(array: Array[Double]): MyDbl2Arr = new MyDbl2Arr(array)
  override def elemFromDbls(d1: Double, d2: Double): MyDbl2Elem = MyDbl2Elem(d1, d2)
  override def fElemStr: MyDbl2Elem => String = _.toString
}

object MyDbl2Arr extends CompanionSlDbl2[MyDbl2Elem, MyDbl2Arr]
{
  implicit val flatImplicit: BuilderArrFlat[MyDbl2Arr] = new BuilderFlatArrDbl2[MyDbl2Arr]
  { type BuffT = MinesBuff
    override def fromDblArray(array: Array[Double]): MyDbl2Arr = new MyDbl2Arr(array)
