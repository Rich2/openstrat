...
object Slate2
{ given transSimerImplicit[T <: SimilarPreserve]: Slate2[T] = (obj, op) => obj.slate(op).asInstanceOf[T]

  /** Implicit [[SlateXY]] instance / evidence for [[RArr]]. */
  given rArrImplicit[A](using ev: Slate2[A]): Slate2[RArr[A]] = (obj, op) => obj.smap(ev.slate(_, op))

  /** Implicit [[SlateXY]] instance / evidence for [[Functor]]. This provides instances for List, Option etc. */
  given functorImplicit[A, F[_]](using evF: Functor[F], evA: Slate2[A]): Slate2[F[A]] = (obj, op) => evF.mapT(obj, evA.slate(_, op))

  /** Implicit [[SlateXY]] instance / evidence for [[Array]]. */
  given arrayImplicit[A](using ct: ClassTag[A], ev: Slate2[A]): Slate2[Array[A]] = (obj, op) => obj.map(ev.slate(_, op))
}
...
implicit class SlateExtensions[A](value: A)(implicit ev: Slate2[A])
