...
{ type ThisT <: TransRigidDister
  ...
  def mirrorY: ThisT = mirrorYOffset(0.metre)
  def mirrorX: ThisT = mirrorXOffset(0.metre)
  def ySlate(yDelta: Metre): ThisT = slate(PtM2(0.metre, yDelta))

  /** Translate in 2 dimensional space. */
  def slate(xOffset: Metre, yOffset: Metre): ThisT = slate(PtM2(xOffset, yOffset))
}
...
{ /** Translate 2 dimensional vectors along the X axis */
  def slateX(xOffset: Metre): T = ev.slate(value, PtM2(xOffset, 0.metre))

  /** Translate 2 dimensional vectors along the Y axis */
  def slateY(yOffset: Metre): T = ev.slate(value, PtM2(0.metre, yOffset))

  /** Translate in 2 dimensional space. */
  def slate(offset: PtM2): T = ev.slate(value, offset)

  /** Translate in 2 dimensional space. */
  def slate(xOffset: Metre, yOffset: Metre): T = ev.slate(value, PtM2(xOffset, yOffset))
}