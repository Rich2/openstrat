...
{ type ThisT <: TransRigidDister
  ...
  def ySlate(yDelta: Metres): ThisT = slate(PtM2(0.metres, yDelta))

  /** Translate in 2 dimensional space. */
  def slate(xOffset: Metres, yOffset: Metres): ThisT = slate(PtM2(xOffset, yOffset))
}
...
{ /** Translate 2 dimensional vectors along the X axis */
  def slateX(xOffset: Metres): T = ev.slate(value, PtM2(xOffset, 0.metres))

  /** Translate 2 dimensional vectors along the Y axis */
  def slateY(yOffset: Metres): T = ev.slate(value, PtM2(0.metres, yOffset))
