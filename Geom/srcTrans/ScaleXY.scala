...
trait ScaleXY[T]
{ /** XY scaling 2D geometric transformation on an object of type T, returns a new type T. This allows different scaling factors across X and Y dimensions. For
   * many instances the implementation of this method will delegate to the object itself. This is an affine transformation, but it is not a similar
   * transformation.*/
...
object ScaleXY
{
  implicit def arrImplicit[A, AA <: Arr[A]](using build: BuilderMapArr[A, AA], ev: ScaleXY[A]): ScaleXY[AA] =
...
{ /** 2d geometric scale transformation extension method, where X and Y axes can be scaled independently for type T, returning a type T. This is an affine
  ...
  /** scale X axis 2d geometric transformation extension method, for type T, returning a type T. This is an affine transformation, but it is not a similar
   * transformation. */
