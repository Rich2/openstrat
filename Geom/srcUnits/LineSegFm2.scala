...
{ def xStart: Femtometres = Femtometres(xStartFemtometresNum)
  ...
  override def mapGeom2(operand: Length): LineSeg = LineSeg(xStartFemtometresNum / operand.femtometresNum, yStartFemtometresNum / operand.femtometresNum,
...
class LineSegFm2Arr(val arrayUnsafe: Array[Double]) extends Dbl4Arr[LineSegFm2]
...
{ override def fromArray(array: Array[Double]): LineSegFm2Arr = new LineSegFm2Arr(array)
  ...
  implicit val flatBuildEv: BuilderArrFlat[LineSegFm2Arr] = new BuilderArrDbl4Flat[LineSegFm2Arr]
  { type BuffT = LineSegFm2Buff
    override def fromDblArray(array: Array[Double]): LineSegFm2Arr = new LineSegFm2Arr(array)
...
class LineSegFm2Buff(val unsafeBuffer: ArrayBuffer[Double]) extends AnyVal, BuffDbl4[LineSegFm2]
