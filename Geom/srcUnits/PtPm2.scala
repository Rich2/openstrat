...
{ /** Factory method for creating a 2-dimensional point measured in picometres from the X and Y [[Length]]s. There is an apply name overload that takes the
  ...
  /** Implicit [[BuilderLinePathLikeMap]] type class instance / evidence for [[PtPm2]] and [[LinePathPm2]]. */
  given linePathBuilderEv: BuilderLinePathDbl2Map[PtPm2, LinePathPm2] = new BuilderLinePathDbl2Map[PtPm2, LinePathPm2]
...
object PtPm2Arr extends CompanionSlDbl2[PtPm2, PtPm2Arr]
{ override def fromArray(array: Array[Double]): PtPm2Arr = new PtPm2Arr(array)
...
{ override def typeStr: String = "VecPm2"
  ...
  override def mapGeom2(operator: Length): Vec2 = Vec2(xPicometresNum / operator.picometresNum, yPicometresNum / operator.picometresNum)
}
...
{ override def typeStr: String = "VecPm2Buff"
  def elemFromDbls(d1: Double, d2: Double): VecPm2 = VecPm2(d1, d2)
}