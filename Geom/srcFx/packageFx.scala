...
{ val userHomeDir: String = System.getProperty("user.home")
  ...
  def findDevSettingT[A: PersistPrecision](settingStr: String): EMon[A] = devSettingsStatements.flatMap(_.findSetting(settingStr))

  /** Find a setting of the given name and type from the file DevSettings.rson, else return the given default value.. */
  def findDevSettingElse[A: PersistPrecision](settingStr: String, elseValue: => A): A = devSettingsStatements.flatMap(_.findSetting(settingStr)).getElse(elseValue)
  ...
  def fromRsonFileFind[A: PersistPrecision](fileName: String): EMon[A] = loadRsonFile(fileName).findType[A]
  def fromRsonFileFindElse[A: PersistPrecision](fileName: String, elseValue: => A): A = fromRsonFileFind(fileName).getElse(elseValue)
  ...
  def fromRsonFileFindForeach[A: PersistPrecision](fileName: String, f: A => Unit): Unit = fromRsonFileFind(fileName).forGood(f)
  def settFromFile[A: PersistPrecision](settingStr: String, fileName: String): EMon[A] = loadRsonFile(fileName).findSetting[A](settingStr)
  def settFromFileElse[A: PersistPrecision](settingStr: String, fileName: String, elseValue: A): A = settFromFile[A](settingStr, fileName).getElse(elseValue)
}
