...
{ val userHomeDir: String = System.getProperty("user.home")
  ...
  def findDevSettingT[A: PersistPrec](settingStr: String): EMon[A] = devSettingsStatements.flatMap(_.findSetting(settingStr))

  /** Find a setting of the given name and type from the file DevSettings.rson, else return the given default value.. */
  def findDevSettingElse[A: PersistPrec](settingStr: String, elseValue: => A): A = devSettingsStatements.flatMap(_.findSetting(settingStr)).getElse(elseValue)
  ...
  def fromRsonFileFind[A: PersistPrec](fileName: String): EMon[A] = loadRsonFile(fileName).findType[A]
  def fromRsonFileFindElse[A: PersistPrec](fileName: String, elseValue: => A): A = fromRsonFileFind(fileName).getElse(elseValue)
  ...
  def fromRsonFileFindForeach[A: PersistPrec](fileName: String, f: A => Unit): Unit = fromRsonFileFind(fileName).forGood(f)
  def settFromFile[A: PersistPrec](settingStr: String, fileName: String): EMon[A] = loadRsonFile(fileName).findSetting[A](settingStr)
  def settFromFileElse[A: PersistPrec](settingStr: String, fileName: String, elseValue: A): A = settFromFile[A](settingStr, fileName).getElse(elseValue)
}
