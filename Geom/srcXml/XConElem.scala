...
{ /** Returns the XML / HTML source code, formatted according to the input. This allows the XML to be indented according to its context. This will generally use
  ...
  def outLines(indent: Int, line1InputLen: Int, maxLineLen: Int = MaxLineLen): TextLines = TextLines(Array(out(indent, maxLineLen)), 3, 30, 30)
}
...
{ override def out(indent: Int, line1InputLen: Int, maxLineLen: Int = MaxLineLen): String = value
  override def outLines(indent: Int, line1InputLen: Int, maxLineLen: Int): TextLines = TextLines(Array(value), 1, value.length, value.length)
}