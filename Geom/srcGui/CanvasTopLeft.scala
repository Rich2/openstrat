...
trait CanvasTopLeft extends CanvasPlatform
{
  ...
  final override def pShapeGenDraw(sgd: ShapeGenDrawOld): Unit = tlShapeDraw(sgd.negY.slate(width / 2, height / 2))
  ...
  final override def textGraphic(tg: TextGraphic): Unit = tlTextGraphic(tg.negY.slate(width / 2, height / 2))
