...
{ type ThisT <: Square
  ...
  override def slateFrom(xOperand: Double, yOperand: Double): Square = vertsTrans(_.slateFrom(xOperand, yOperand))
  ...
  override def mirror(lineLike: LineLike): Square = vertsTrans(_.mirror(lineLike))
...
{ override type ShapeT = Sqlign
  ...
  def apply(width: Double, rotation: AngleVec, cen: Pt2 = Origin2): Square = {
    val hw = width / 2
  ...
  override def reify(scale: Double, cen: Pt2 = Origin2): Sqlign = Sqlign(scale, cen)
