...
/** A 2D geometric element, that has a defined bounding [[Rect]]. This trait is for layout, such as placing Graphic elements in rows and columns. It includes
 * polygon and shape graphics but not line and curve graphics. */
trait BoundedElem extends Any, Geom2Elem
...
{ /** Implicit [[Bounding]] type class instances evidence for objects that extends [[BoundedElem]]. */
  given boundedEv[A <: BoundedElem]: Bounding[A] = obj => obj.boundingRect

  /** Implicit [[Bounding]] type class instances evidence for [[Sequ]]. */
  given sequEv[A](using evA: Bounding[A]): Bounding[Sequ[A]] = new Bounding[Sequ[A]]
...
{ /** Extension method gives the X and Y bounds of this object. */
  ...
  def cenDefaultTo(newCenDefault: Pt2)(using ev: Slate2[A]): A = ev.slateT(thisObj, evA.boundCen(thisObj) >> newCenDefault)
  ...
  def boundsCenTo(newCen: Pt2)(using ev: Slate2[A]): A = ev.slateT(thisObj, evA.boundCen(thisObj) >> newCen)
   ...
   * point. */
  def boundsTRTo(newTopRight: Pt2)(using ev: Slate2[A]): A = ev.slateT(thisObj, evA.boundsTR(thisObj) >> newTopRight)
   ...
   * point. */
  def boundsBRTo(newBottomRight: Pt2)(using ev: Slate2[A]): A = ev.slateT(thisObj, evA.boundsBR(thisObj) >> newBottomRight)
   ...
   * point. */
  def boundsBLTo(newBottomLeft: Pt2)(using ev: Slate2[A]): A = ev.slateT(thisObj, evA.boundsBL(thisObj) >> newBottomLeft)
   ...
   * point. */
  def boundsTLTo(newTopLeft: Pt2)(using ev: Slate2[A]): A = ev.slateT(thisObj, evA.boundsTL(thisObj) >> newTopLeft)
}