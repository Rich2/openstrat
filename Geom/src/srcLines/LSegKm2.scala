 ...
 *  surface. */
class LSegKm2(val xStartKilometresNum: Double, val yStartKilometresNum: Double, val xEndKilometresNum: Double, val yEndKilometresNum: Double) extends
  LSegLen2[PtKm2] with LSegDbl4[PtKm2] with Dbl4Elem
{ def xStart: Kilometres = Kilometres(xStartKilometresNum)
  ...
  override def slate(operand: VecPtLen2): LSegKm2 = LSegKm2(xStartKilometresNum + operand.xKilometresNum, yStartKilometresNum + operand.yKilometresNum,
    xEndKilometresNum + operand.xKilometresNum, yEndKilometresNum + operand.yKilometresNum)

  override def slate(xOperand: Length, yOperand: Length): LSegKm2 = LSegKm2(xStartKilometresNum + xOperand.kilometresNum,
  ...
  override def slateX(xOperand: Length): LSegKm2 = LSegKm2(xStartKilometresNum + xOperand.kilometresNum, yStartKilometresNum,
    xEndKilometresNum + xOperand.kilometresNum, yEndKilometresNum)

  override def slateY(yOperand: Length): LSegKm2 = LSegKm2(xStartKilometresNum, yStartKilometresNum + yOperand.kilometresNum, xEndKilometresNum,
    yEndKilometresNum + yOperand.kilometresNum)

  override def scale(operand: Double): LSegKm2 =
    LSegKm2(xStartKilometresNum * operand, yStartKilometresNum * operand, xEndKilometresNum * operand, yEndKilometresNum * operand)
 ...
 * extension method. */
object LSegKm2
{ /** Factory apply method for constructing [[LSegKm2]]s from the start and end points. There is an apply overload to construct from the X and Y components
   * of the start and end points. To construct from scalar quantities use the metresNum method. */
  def apply(startPt: PtLen2, endPt: PtLen2): LSegKm2 = new LSegKm2(startPt.xMetresNum, startPt.yMetresNum, endPt.xMetresNum, endPt.yMetresNum)

  /** Factory apply method for constructing [[LSegKm2]]s from the X and Y components of the start and end points. There is an apply overload to construct
  ...
  def apply(xStartPt: Length, yStartPt: Length, xEndPt: Length, yEndPt: Length): LSegKm2 =
    new LSegKm2(xStartPt.kilometresNum, yStartPt.kilometresNum, xEndPt.kilometresNum, yEndPt.kilometresNum)
  ...
  def apply(xStartKilometresNum: Double, yStartKilometresNum: Double, xEndKilometresNum: Double, yEndKilometresNum: Double): LSegKm2 =
    new LSegKm2(xStartKilometresNum, yStartKilometresNum, xEndKilometresNum, yEndKilometresNum)

  /** [[Show]] and [[Unshow]] type class instances / evidence for [[LSegKm2]]. */
  implicit val persistEv: Persist2Both[PtKm2, PtKm2, LSegKm2] =
    Persist2Both[PtKm2, PtKm2, LSegKm2]("LineSegKM2", "start", _.startPt, "end", _.endPt, LSegKm2(_, _))

  /** Implicit instance / evidence for [[BuilderArrMap]] type class. */
  implicit val buildEv: BuilderArrDbl4Map[LSegKm2, LineSegKm2Arr] = new BuilderArrDbl4Map[LSegKm2, LineSegKm2Arr]
...
/** Compact immutable Array[Double] based collection class for [[LSegKm2]]s. A mathematical straight line segment measured in [[Kilometres]]. */
class LineSegKm2Arr(val arrayUnsafe: Array[Double]) extends AnyVal, LineSegLen2Arr[PtKm2], KilometresBased, ArrDbl4[LSegKm2]
{ type ThisT = LineSegKm2Arr
  ...
  override def fElemStr: LSegKm2 => String = _.toString
  override def elemFromDbls(d1: Double, d2: Double, d3: Double, d4: Double): LSegKm2 = new LSegKm2(d1, d2, d3, d4)
...
/** Companion object for the [[LSegKm2]]s class. */
object LineSegKm2Arr extends CompanionSeqLikeDbl4[LSegKm2, LineSegKm2Arr]
{
  ...
  implicit val flatBuildEv: BuilderArrFlat[LineSegKm2Arr] = new BuilderFlatArrDbl4[LineSegKm2Arr]
  { type BuffT = LineSegKm2Buff
    override def fromDblArray(array: Array[Double]): LineSegKm2Arr = new LineSegKm2Arr(array)
  ...
  implicit lazy val showEv: ShowSequ[LSegKm2, LineSegKm2Arr] = ShowSequ[LSegKm2, LineSegKm2Arr]()

  /** [[Unshow]] type class instance / evidence for [[LineSegKm2Arr]]. */
  implicit lazy val unshowEv: UnshowSeq[LSegKm2, LineSegKm2Arr] = UnshowSeq[LSegKm2, LineSegKm2Arr]()
...
/** Efficient expandable buffer for [[LSegKm2]]s. */
class LineSegKm2Buff(val bufferUnsafe: ArrayBuffer[Double]) extends AnyVal with BuffDbl4[LSegKm2]
