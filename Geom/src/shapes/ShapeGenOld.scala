...
{ type ThisT = ShapeGenOld
  ...
  override def sdElem(iMatch: Double, d1: Double, d2: Double, d3: Double, d4: Double, d5: Double, d6: Double): CurveTail =
    CurveTail(iMatch, d1, d2, d3, d4, d5, d6)

  def ptsTrans(f: Pt2 => Pt2): ShapeGenOld =
  { val newArray = new Array[Double](sdLength * 7)
    ...
    (0 until sdLength).foreach{ index =>
      val offset = index * 7
  ...
  def tailForeach(fLineSeg: CurveTail => Unit, fArcSeg: CurveTail => Unit, fBezierSeg: CurveTail => Unit): Unit =
    dataForeach(_.segDo(fLineSeg, fArcSeg, fBezierSeg))

  @inline def segLast: CurveTail = dsLast
}
