...
{ 
  ...
  override def attribs: RArr[XAtt] = RArr(fillAttrib)
...
{ /** Implicit [[EqT]] type class instance / evidence for [[Colour]]. */
  ...
  { override def typeStr: String = "Colour"
    ...
    override def fromExpr(expr: Expr): ParseExcEither[Colour] =  expr match {
      ...
      case AlphaBracketExpr(IdentUpperToken(_, "Colour"), Arr1(BracketedStructure(Arr1(st), Parentheses, _, _))) => st.expr match {
        ...
        case _ => expr.exprParseErr[Colour](using this)
      }
      case _ => expr.exprParseErr[Colour](using this)
    }
  ...
  implicit val arrBuildImplicit: BuilderArrMap[Colour, ColourArr] = new BuilderMapArrInt1[Colour, ColourArr]
  ...
  def strElse(inp: Colour, elseStr: String): String = optStr(inp).getOrElse(elseStr)
   ...
   /** named colors & values following CSS Color Module Level 4 - though names are UpperCamelCase here - plus 4 non standard colors: BrightSkyBlue,
    * DarkYellow, LemonLime, LightRed*/
