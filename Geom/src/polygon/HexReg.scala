...
{ override def typeStr = "HexReg"
  ...
  override def vertsForeach[U](f: Pt2 => U): Unit = { f(v0); f(v1); f(v2); f(v3); f(v5); f(v6); () }

  override def foreachVertTail[U](f: Pt2 => U): Unit = { f(v1); f(v2); f(v3); f(v5); f(v6); () }
  override def foreachVertPairTail[U](f: (Double, Double) => U): Unit = { f(v0x, v0y);  f(v1x, v1y); f(v2x, v2y);  f(v3x, v3y); f(v5x, v5y); f(v6x, y6); () }

  override def vertsArrayX: Array[Double] = Array(v0x, v1x, v2x, v3x, v5x, v6x)

  override def vertsArrayY: Array[Double] = Array(v0y, v1y, v2y, v3y, v5y, y6)
