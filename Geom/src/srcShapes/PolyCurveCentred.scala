...
{ override type ThisT = PolyCurveCentred
  ...
  def parentAll(evObj: AnyRef, fillColour: Colour, lineWidth: Double, lineColour: Colour, textSize: Int, str: String, textAlign: TextAlign = CenAlign):
    PolyCurveParentFull = PolyCurveParentFull(cen, segs, evObj, RArr(PolyCurveFillDraw(segs, fillColour, lineColour, lineWidth), TextFixed(str, textSize, cen,
    lineColour, textAlign)))
