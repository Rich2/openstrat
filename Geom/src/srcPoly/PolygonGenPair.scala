...
{ given buildEv[A2](using ct2: ClassTag[A2]): BuilderArrMap[PolygonGenPair[A2], PolygonGenPairArr[A2]] = new PolygonGenPairBuilder[A2]
}

final class PolygonGenPairArr[A2](val a1ArrayDbls: Array[Array[Double]], val a2Array: Array[A2]) extends PolygonLikeDbl2PairArr[Pt2, PolygonGen, PolygonGenArr,
  A2, PolygonGenPair[A2]]
{ override type ThisT = PolygonGenPairArr[A2]
  override def setElemUnsafe(index: Int, newElem: PolygonGenPair[A2]): Unit = { a1ArrayDbls(index) = newElem.a1ArrayDbl; a2Array(index) = newElem.a2 }
  ...
  override def elem(index: Int): PolygonGenPair[A2] = new PolygonGenPair[A2](a1ArrayDbls(index), a2Array(index))
...
final class PolygonGenPairBuilder[A2](implicit val b2ClassTag: ClassTag[A2], @unused notB: Not[SpecialT]#L[A2]) extends PolygonLikeDblNPairArrBuilder[Pt2,
  PolygonGen, PolygonGenArr, A2, PolygonGenPair[A2], PolygonGenPairArr[A2]]
{ override type BuffT = PolygonGenPairBuff[A2]
  ...
  { seqLike.a1ArrayDbls(index) = newElem.a1ArrayDbl
  ...
  override def b1Builder: BuilderPolygonLikeMap[Pt2, PolygonGen] = Pt2.polygonMapBuildEv
...
class PolygonGenPairBuff[A2](val b1Buffer: ArrayBuffer[Array[Double]], val b2Buffer: ArrayBuffer[A2]) extends BuffPairSeqLikeDblN[Pt2, PolygonGen, A2,
  PolygonGenPair[A2]]
{ override type ThisT = PolygonGenPairBuff[A2]
  override def setElemUnsafe(index: Int, newElem: PolygonGenPair[A2]): Unit = { b1Buffer(index) = newElem.a1ArrayDbl; b2Buffer(index) = newElem.a2 }
  override def fElemStr: PolygonGenPair[A2] => String = _.toString
  ...
  override def elem(index: Int): PolygonGenPair[A2] = new PolygonGenPair[A2](b1Buffer(index), b2Buffer(index))
}
