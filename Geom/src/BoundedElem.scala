...
{ /** 2D geometric translation transformation on this type T, returning an object of type T with its default centre at the parameter point. */
  def cenDefaultTo(newCenDefault: Pt2)(implicit ev: Slate[T]): T = ev.slate(thisT, thisT.cenDefault >> newCenDefault)
}
...
class BoundingExtensions[A](val thisObj: A, evA: Bounding[A])
{
  ...
  def boundsCenTo(newCen: Pt2)(implicit ev: Slate[A]): A = ev.slate(thisObj, evA.boundCen(thisObj) >> newCen)
   ...
   * point. */
  def boundsTRTo(newTopRight: Pt2)(implicit ev: Slate[A]): A = ev.slate(thisObj, evA.boundsTR(thisObj) >> newTopRight)
   ...
   * point. */
  def boundsBRTo(newBottomRight: Pt2)(implicit ev: Slate[A]): A = ev.slate(thisObj, evA.boundsBR(thisObj) >> newBottomRight)
   ...
   * point. */
  def boundsBLTo(newBottomLeft: Pt2)(implicit ev: Slate[A]): A = ev.slate(thisObj, evA.boundsBL(thisObj) >> newBottomLeft)
   ...
   * point. */
  def boundsTLTo(newTopLeft: Pt2)(implicit ev: Slate[A]): A = ev.slate(thisObj, evA.boundsTL(thisObj) >> newTopLeft)
}