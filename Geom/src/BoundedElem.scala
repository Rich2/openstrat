...
trait BoundedElem extends Any, Axlign2Elem
...
{ /** Extension method gives the X and Y bounds of this object. */
  ...
  def cenDefaultTo(newCenDefault: Pt2)(using ev: Slate2[A]): A = ev.slate(thisObj, evA.boundCen(thisObj) >> newCenDefault)
  ...
  def boundsCenTo(newCen: Pt2)(using ev: Slate2[A]): A = ev.slate(thisObj, evA.boundCen(thisObj) >> newCen)
   ...
   * point. */
  def boundsTRTo(newTopRight: Pt2)(using ev: Slate2[A]): A = ev.slate(thisObj, evA.boundsTR(thisObj) >> newTopRight)
   ...
   * point. */
  def boundsBRTo(newBottomRight: Pt2)(using ev: Slate2[A]): A = ev.slate(thisObj, evA.boundsBR(thisObj) >> newBottomRight)
   ...
   * point. */
  def boundsBLTo(newBottomLeft: Pt2)(using ev: Slate2[A]): A = ev.slate(thisObj, evA.boundsBL(thisObj) >> newBottomLeft)
   ...
   * point. */
  def boundsTLTo(newTopLeft: Pt2)(using ev: Slate2[A]): A = ev.slate(thisObj, evA.boundsTL(thisObj) >> newTopLeft)
}