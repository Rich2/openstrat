...
{ type ThisT <: TransSimDisterUser
  ...
  override def slate(offset: PtM2): ThisT = newThis(geomMem.slate(offset).asInstanceOf[MemT])
  override def rotateRadians(radians: Double): ThisT = newThis(geomMem.rotateRadians(radians).asInstanceOf[MemT])
  override def mirrorYOffset(xOffset: Length): ThisT = newThis(geomMem.mirrorYOffset(xOffset).asInstanceOf[MemT])
  override def mirrorXOffset(yOffset: Length): ThisT = newThis(geomMem.mirrorXOffset(yOffset).asInstanceOf[MemT])
...
object TransSimDist
{
  ...
  implicit def arrImplicit[A, AA <: SeqImut[A]](implicit build: ArrBuilder[A, AA], ev: TransSimDist[A]): TransSimDist[AA] = new TransSimDist[AA]
