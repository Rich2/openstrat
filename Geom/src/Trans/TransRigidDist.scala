...
{ type ThisT <: TransRigidDister
  ...
  def ySlate(yDelta: Length): ThisT = slate(PtMetre2(0.metres, yDelta))

  /** Translate in 2 dimensional space. */
  def slate(xOffset: Length, yOffset: Length): ThisT = slate(PtMetre2(xOffset, yOffset))
}
...
object TransRigidDist
{
  ...
  implicit def arrImplicit[A, AA <: SeqImut[A]](implicit build: ArrBuilder[A, AA], ev: TransRigidDist[A]): TransRigidDist[AA] = new TransRigidDist[AA]
...
{ /** Translate 2 dimensional vectors along the X axis */
  def slateX(xOffset: Length): T = ev.slate(value, PtMetre2(xOffset, 0.metres))

  /** Translate 2 dimensional vectors along the Y axis */
  def slateY(yOffset: Length): T = ev.slate(value, PtMetre2(0.metres, yOffset))

  /** Translate in 2 dimensional space. */
  def slate(offset: PtMetre2): T = ev.slate(value, offset)

  /** Translate in 2 dimensional space. */
  def slate(xOffset: Length, yOffset: Length): T = ev.slate(value, PtMetre2(xOffset, yOffset))
}