...
{ override def contents: RArr[XConCompound] = RArr(HtmlH2("Polygons"), p1, svg1, p2, code1, p3, svg2, p4)
  ...
  val svg1: SvgSvgElem = SvgSvgElem(dodec1.boundingRect.addMargin(GeomPage.svgMargin), clock, RArr(CentreBlockAtt))
  ...
  val svg2: SvgSvgElem = SvgSvgElem(rect1.boundingRect.addMargin(GeomPage.svgMargin).addHorrMargin(200), RArr(rect2, cen) ++ verts2 ++ sides2, RArr(CentreBlockAtt))
