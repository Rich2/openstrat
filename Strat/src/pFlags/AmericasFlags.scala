...
{ val name = "United States"
  ...
  val star0 = Star5().scaleOld(starScale).fill(White)
  val star = star0.slateOld(-0.95, 0.5)

  val apply =
  { val blueFieldOld = Rectangle.fromTL(0.76, 7.0/ 13, -0.95 vv 0.5).fill(usBlue)
    val stars = ijToMap(0, 10, 2)(1, 9, 2) { (x, y) => star.slateOld(starX + x * starX, -y * starY) }
    val starsInner = ijToMap(2, 10, 2)(2, 8, 2) { (x, y) => star.slateOld(x * starX, -y * starY) }
 ...
 val apply: Arr[PaintFullElem] =
  {
    Arr[PaintFullElem](
      ...
      Star5().scaleOld(0.125).slateOld(-0.5 vv 0.25).fill(White)
    )
  }
}
