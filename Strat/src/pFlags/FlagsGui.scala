...
case class FlagsGui(canv: CanvasPlatform) extends CanvasNoPanels("Flags Gui")
{
  ...
  val tlObjs = tlFlags.iMap((el, i) => el.parent.scale(100).topLeft.slate(canv.topLeft.subY(i * 110)))

  val trFlags = Refs(Belgium, Chad, France)
  val trObjs = trFlags.iMap((el, i) => el.parent.scale(100).topRight.slate(canv.topRight.subY(i * 110)))

  val blFlags = Refs(China, Italy)
  val blObjs = blFlags.iMap((el, i) => el.parent.scale(100).bottomLeft.slate(canv.bottomLeft.addY(i * 110)))

  val brFlags = Refs(Germany, Germany1871, Ireland)
  val brObjs = brFlags.iMap((el, i) => el.parent.scale(100).bottomRight.slate(canv.bottomRight.addY(i * 110)))
  ...
  val us = UnitedStates.parent.scale(200).slate(-300, 350)
  val stuff = tlObjs ++ trObjs ++ blObjs ++ brObjs -+ us -++ star ++ cr
