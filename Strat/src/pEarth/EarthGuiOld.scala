...
abstract class EarthGuiOld(title: String) extends UnfixedMapGui(title)
{
 ...
 //  val transSeq: Dist2s => Vec2s = _.map(trans)
  ...
  val bInv: DisplayBoundedFull = clickButton("inv", invCmd)
  ...
  def eaButts: Arr[DisplayBoundedFull] =  Arr(bSave, bLoad)
  def cmd00: MouseCmd = mb => { focus = LatLong0; focusUp = true; updateView }
  def b00: DisplayBoundedFull = clickButton("00", cmd00)
  ...
  def blueCircle = CircleOld.segs(EarthAvDiameter / scale).fill(Colour.DarkBlue)
  def redCircle = CircleOld.segs(EarthAvDiameter / scale).draw(1, Colour.Red)
  def mapObjs: GraphicElemFulls = ls //:+ hairs
