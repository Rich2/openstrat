...
class UnusSetGui(val canv: CanvasPlatform, val grid: UnusGrid) extends TileGridGui[UTile, SideBare, UnusGrid]("Unus Game")
{
  ...
  override def mapObjs = tilesCoodFlatMapListAll{c => List(/*tileActiveOnly(c, c),*/ coodStrDisp(c)) } :::
  grid.tilesOptionDispAll(t => t.oPlayer.map(p => Rectangle(120, 80, coodToDisp(t.cood)).fillDrawSubj(p, p.colour, 2)))  :::  sidesDrawAll()
