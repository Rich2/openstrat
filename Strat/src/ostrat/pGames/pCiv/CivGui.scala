...
class CivGui(canv: CanvasPlatform) extends HexGridGui[CTile, SideBare, CivGrid](canv, "Civs")
{
    ...
    case (RightButton, List(warr : Warrior), List(newTile: CTile)) =>
      {
        ...
        if (HexGridComplex.adjTileCoodsOfTile(oldCood).contains(newCood) && warr.movePts > 0)
        {
  ...
  def turnCmd: MB0 = mb => {tilesForeach(_.lunits.foreach(_.resetMovePts())); repaintMap }
