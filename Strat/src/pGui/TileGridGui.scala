...
package ostrat
package pGui
import pCanv._, geom._, reflect.ClassTag, pGrid._
...
abstract class TileGridGui[TileT <: TileOld, SideT <: TileSideOld, GridT <: TileGridRegOld[TileT, SideT]](title: String) extends UnfixedMapGui(title)
{
 ...
 // final def sidesDrawAllOld(lineWidth: Double = 2, colour: Colour = Colour.Black): GraphicElemsOld = sideLinesDispAll.mapArrSeq(_.draw(lineWidth, colour))
  ...
  def tilesFlatMapAll[B, BB <: ArrImut[B]](f: TileT => BB)(implicit build: ArrBuild[B, BB]): BB = grid.tilesFlatMapAll[B, BB](f)
