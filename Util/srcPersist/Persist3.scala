...
trait Show3[A1, A2, A3] extends Any with ShowNed with PersistBase3[A1, A2, A3]
{ override def opt1: Option[A1] = None
  ...
  override def paramNames: StrArr = StrArr(name1, name2, name3)
  override def elemTypeNames: StrArr = StrArr(persist1.typeStr, persist2.typeStr, showT3.typeStr)

  override def showElemStrs(way: ShowStyle): StrArr = StrArr(persist1.showT(show1, way), persist2.showT(show2, way), showT3.showT(show3, way))
...
{ final override def syntaxDepth: Int = 2
  ...
  final override implicit def persist2: Persist[Double] = ShowT.doublePersistEv
...
object Unshow3
{
  ...
  class Unshow3Imp[A1, A2, A3, R](val typeStr: String, val name1: String, val name2: String, val name3: String, val newT: (A1, A2, A3) => R,
    ...
    implicit val persist1: Unshow[A1], val persist2: Unshow[A2], val ev3: Unshow[A3]) extends Unshow3[A1, A2, A3, R]
  {
...
object Persist3
{
  ...
  class Persist3Imp[A1, A2, A3, R](val typeStr: String, val name1: String, val fArg1: R => A1, val name2: String, val fArg2: R => A2,
    ...
    opt1In: Option[A1] = None)(implicit val persist1: Persist[A1], val persist2: Persist[A2], val ev3: Persist[A3]) extends Persist3[A1, A2, A3, R]
  { val opt2: Option[A2] = ife(opt3.nonEmpty, opt2In, None)
    val opt1: Option[A1] = ife(opt2.nonEmpty, opt1In, None)
...
{ /** Factory apply method for [[PersistShow3]], that Persists [[ShowDec3]] objects. */
  ...
  class PersistShow3Imp[A1, A2, A3, R <: Show3[A1, A2, A3]](val typeStr: String, val name1: String, val name2: String, val name3: String,
    ...
    implicit val persist1: Persist[A1], val persist2: Persist[A2], val ev3: Persist[A3]) extends PersistShow3[A1, A2, A3, R]
  { val opt2: Option[A2] = ife(opt3.nonEmpty, opt2In, None)
    val opt1: Option[A1] = ife(opt2.nonEmpty, opt1In, None)
  }
}
...
{ val opt2: Option[Double] = ife(opt3.nonEmpty, opt2In, None)
  ...
  override def persist2: Persist[Double] = ShowT.doublePersistEv
