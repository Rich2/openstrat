...
{ /** Adds any object' with an implicit instance in scope's String representation to this Rval's String. */
  def - [A](value: A)(implicit ev: Showing[A]): Rval = new Rval(str + "\n" + ev.strT(value) + ";")
}

object Rval
{ /** Factory apply method for creating initial Rval box. */
  def apply[A](value: A)(implicit ev: Showing[A]): Rval = new Rval(ev.strT(value) + ";")
}
...
class Setting(val str: String) extends AnyVal
{
  ...
  def ap[A](setting: String, value: A)(implicit ev: Showing[A]): Setting =
  {
...
object Setting
{
  def apply[A](setting: String, value: A)(implicit ev: Showing[A]): Setting = new Setting(setting + " = " + ev.strT(value) + ";")
}
