...
trait Tell3Plused[A1, A2, A3] extends Any, Tell2Plused[A1, A2], Persist3Plus[A1, A2, A3]
 ...
 * [[Show3]] and then use [[Show3]] or [[Persist3Both]] to create the type class instance for Show. The [[Show3]] or [[Persist3Both]] class will delegate to
 * [[Show3]] for some of its methods. */
...
object ShowTell3
{
  def apply[A1, A2, A3, R <: Tell3[A1, A2, A3]](typeStr: String)(using ev1: Show[A1], ev2: Show[A2], ev3: Show[A3]): ShowTell3[A1, A2, A3, R] =
    new ShowTell3Imp[A1, A2, A3, R](typeStr)

  /** Implementation class for the general cases of the [[ShowTell3]] trait. */
  class ShowTell3Imp[A1, A2, A3, R<: Tell3[A1, A2, A3]](val typeStr: String)(using val show1: Show[A1], val show2: Show[A2], val show3: Show[A3]) extends
...
class PersistTell3[A1, A2, A3, A <: Tell3[A1, A2, A3]](val typeStr: String, val name1: String, val name2: String, val name3: String,
  val shortKeys: ArrPairStr[A], val newT: (A1, A2, A3) => A, override val opt3: Option[A3], opt2In: Option[A2], opt1In: Option[A1])(using
  val unshow1Ev: Unshow[A1], val unshow2Ev: Unshow[A2], val unshow3Ev: Unshow[A3]) extends PersistTell[A], ShowTell3[A1, A2, A3, A], Unshow3[A1, A2, A3, A]
{ override val opt2: Option[A2] = ife(opt3.nonEmpty, opt2In, None)
  override val opt1: Option[A1] = ife(opt2.nonEmpty, opt1In, None)
}

object PersistTell3
{ /** Factory apply method for creating [[PersistTell3]] type class instances / evidence. */
  def apply[A1, A2, A3, A <: Tell3[A1, A2, A3]](typeStr: String, name1: String, name2: String, name3: String, newT: (A1, A2, A3) => A, opt3: Option[A3] = None,
    opt2: Option[A2] = None, opt1: Option[A1] = None)(using unshow1Ev: Unshow[A1], unshow2Ev: Unshow[A2], unshow3Ev: Unshow[A3], classTag: ClassTag[A]):
    PersistTell3[A1, A2, A3, A] = new PersistTell3[A1, A2, A3, A](typeStr, name1, name2, name3, ArrPairStr[A](), newT, opt3, opt2, opt1)

  /** Factory method for creating [[PersistTell3]] type class instances / evidence with short labels. */
  ...
  def explicit[A1, A2, A3, A <: Tell3[A1, A2, A3]](typeStr: String, name1: String, name2: String, name3: String, newT: (A1, A2, A3) => A, unshow1Ev: Unshow[A1],
    ...
    new PersistTell3[A1, A2, A3, A](typeStr, name1, name2, name3, ArrPairStr[A](), newT, opt3, opt2, opt1)(using unshow1Ev, unshow2Ev, unshow3Ev)
}