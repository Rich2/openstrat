...
{ /** Allows this [[Unshow]] instance to create object from it's 6 components. */
  ...
  { val len: Int = sortedExprs.length
    ...
    def e5: ExcEither[A5] = ife(len > pSeq(4), unshow5.fromSettingOrExpr(name5, sortedExprs(pSeq(4))), opt5.toErrBi)
    def e6: ExcEither[A6] = ife(len > pSeq(5), unshow6.fromSettingOrExpr(name6, sortedExprs(pSeq(5))), opt6.toErrBi)
