...
object parse7Clauses
{
  ...
  def apply (implicit seg: RArr[ColonOpMem]): Either[AstException, ColonMemExpr] = fromOffset(seg.offset0)

  def fromOffset(inp: ArrOff[ColonOpMem])(implicit seg: RArr[ColonOpMem]): Either[AstException, ColonMemExpr] =
  {
    var subAcc: ArrayBuffer[ClauseMem] = Buffer()
    val acc: ArrayBuffer[Clause] = Buffer()

    def loop(rem: ArrOff[ColonOpMem]): Either[AstException, ColonMemExpr] = rem match
