...
{ /** Function apply method parses assignment expressions. */
  def apply(using inp: RArr[StatementMem]): Either[ExcAst, Expr] =
  { val leftAcc: ArrayBuffer[AssignMem] = Buffer()
    val rightAcc: ArrayBuffer[AssignMem] = Buffer()

    def leftLoop(rem: ArrOff[StatementMem]): Either[ExcAst, Expr] = rem match
    { case ArrOff0() => parse6ColonExpr(using leftAcc.toArr)
    ...
    def rightLoop(rem: ArrOff[StatementMem])(using seg: RArr[StatementMem]): Either[ExcAst, AssignMemExpr] = rem match
