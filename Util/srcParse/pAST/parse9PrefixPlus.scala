...
{ /** Seems to parse prefix operators. Function object apply method. */
  def apply(implicit refs: RArr[ClauseMem]): EEArr[ClauseMem] =
  {
    val acc: ArrayBuffer[ClauseMem] = Buffer()

    def loop(rem: ArrOff[ClauseMem]): EEArr[ClauseMem] = rem match
    { case ArrOff0() => Succ(acc).map(_.toArr)
      ...
      case ArrOffHead(pp: OperatorToken) => Fail("Prefix operator not followed by expression")
