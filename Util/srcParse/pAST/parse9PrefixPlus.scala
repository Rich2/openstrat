...
{ /** Seems to parse prefix operators. Function object apply method. */
  def apply(implicit refs: RArr[ClauseMem]): ErrBiArr[Exception, ClauseMem] =
  {
    val acc: ArrayBuffer[ClauseMem] = Buffer()

    def loop(rem: ArrOff[ClauseMem]): ErrBiArr[Exception, ClauseMem] = rem match
    { case ArrOff0() => Succ(acc).map(_.toArr)
      ...
      case ArrOffHead(pp: OperatorToken) => pp.startPosn.failLexar("Prefix operator not followed by expression")
