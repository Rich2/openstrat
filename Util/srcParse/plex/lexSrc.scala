...
{ /** lexes [[String]]. */
  ...
  { implicit val charArr: CharArr = new CharArr(charsIn)
    ...
    { case CharsOff0() => acc.succRArr
      ...
      case CharsOffHead4('.', '.', '.', '.') => acc2 = posn.right(4).leftLexar(".... is not an allowed character sequence.")
      ...
      case CharsOff1Tail('\'', _) => acc2 = posn.leftLexar("Unclosed Character literal.")
      ...
      case CharsOffHead(c) => acc2 = posn.leftLexar("Unimplemented character in main loop: " + c.toString)
    }
    acc2.map(_.toArr)
  }
}