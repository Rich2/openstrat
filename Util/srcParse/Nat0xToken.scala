...
{
  /** Function for parsing explicit Hexadecimal Token, one that begins with the characters '0x'. */
  def parse(rem: CharsOff, tp: TextPosn)(implicit charArr: CharArr): EEMon3[CharsOff, TextPosn, Token] =
  {
    def loop(rem: CharsOff, strAcc: String, intAcc: Long): EEMon3[CharsOff, TextPosn, Nat0xToken] = rem match
    ...
    { case CharsOff3Tail('0', 'x', HexaDigitChar(c, i), tail) => loop (tail, c.toString, i)
      case CharsOffHead3('0', 'x', WhitespaceChar(_)) => tp.fail("Empty hexademicmal token.")
      case CharsOffHead3('0', 'x', c) => tp.fail("Badly formed hexademicmal token.")
