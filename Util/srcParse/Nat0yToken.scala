...
{
  /** Function for parsing explicit Hexadecimal Token, one that begins with the charchters '0y'. */
  def parse(rem: CharsOff, tp: TextPosn)(implicit charArr: CharArr): Ebbf3[ExcLexar, CharsOff, TextPosn, Token] =
  {
    def loop(rem: CharsOff, strAcc: String): Ebbf3[ExcLexar, CharsOff, TextPosn, Nat0yToken] = rem match
    ...
    def upperLoop(rem: CharsOff, strAcc: String): Ebbf3[ExcLexar, CharsOff, TextPosn, Nat0yToken] = rem match
    { case CharsOff0() => Succ3(rem, tp.right(strAcc.length + 2), Nat0yToken(tp, strAcc))
    ...
    def lowerLoop(rem: CharsOff, strAcc: String): Ebbf3[ExcLexar, CharsOff, TextPosn, Nat0yToken] = rem match
    { case CharsOff0() => Succ3(rem, tp.right(strAcc.length + 2), Nat0yToken(tp, strAcc))
