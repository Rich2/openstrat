...
object Statement
{
  ...
  { private def ifEmptyTextPosn: TextPosn = TextPosn("Empty Statement Seq", 0, 0)
    ...
    def findSettingInt(settingStr: String): EMon[Int] = Showing.intPersistEv.settingFromStatements(statements, settingStr)

    /** Find Setting of the given name and type [[Double]] from this Arr[Statement] Extension method. */
    def findSettingDbl(settingStr: String): EMon[Double] = Showing.doublePersistEv.settingFromStatements(statements, settingStr)
    ...
    def findSettingBool(settingStr: String): EMon[Boolean] = Showing.booleanPersistImplicit.settingFromStatements(statements, settingStr)
  }
