...
object OtherTest extends TestSuite
{
  val tests = Tests {
    ...
    { OptionMap2(op1, op2)(_.str + "! " + _) ==> Some("5! Wow")
      OptionMap3(op1, op2, op3)((s1, s2, s3) => s1 + s2.length + s3) ==> Some(11.2)
      OptionMap4(op1, op2, op3, op4)((s1, s2, s3, s4) => s1 + s2.length + s3 + s4) ==> None
    }
  }
}