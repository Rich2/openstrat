...
object DoubleTokenTest extends TestSuite
{
  ...
  val tests = Tests {
    "Test1" - {
      ...
      assertMatch(s51.parseTokens){ case Good(Arr1(DeciFracToken(Sp1, "51", "1", ""))) => }
      "51".unsafeDigitsToLong ==> 51l
      s51.findDouble ==> Good(51.1)
      assertMatch(xeqStr.parseTokens){ case Good(Arr3(IdentLowerOnlyToken(_, "x"), AsignToken(_), DeciFracToken(_, _, _, _))) => }
