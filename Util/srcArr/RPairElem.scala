...
/** R for stored by reference. The generalised default [[ArrPair]] for types that do not have their own specialised classes. Note although they are named as
 * reference [[ArrPair]]s the A1 type parameter does not have to inherit from [[AnyRef]]. */
class RArrPair[A1, A2](val a1Array: Array[A1], val a2Array: Array[A2]) extends ArrPair[A1, RArr[A1], A2, RPairElem[A1, A2]]
{ override type ThisT = RArrPair[A1, A2]
  ...
  def replaceA1byA2(key: A2, newValue: A1)(implicit ct1: ClassTag[A1]): RArrPair[A1, A2] =
  { val newA1Array = new Array[A1](length)
    ...
    new RArrPair[A1, A2](newA1Array, a2Array)
  }

  /** Returns a new uninitialised [[RArrPair]]. */
  def uninitialised(length: Int)(implicit cl1: ClassTag[A1], ct2: ClassTag[A2]): RArrPair[A1, A2] =
    new RArrPair[A1, A2](new Array[A1](length), new Array[A2](length))

  @targetName("append") def +%(operand: RPairElem[A1, A2])(implicit ct1: ClassTag[A1], ct2: ClassTag[A2]): RArrPair[A1, A2] =
    appendPair(operand.a1, operand.a2)

  def appendPair(a1: A1, a2: A2)(implicit ct1: ClassTag[A1], ct2: ClassTag[A2]): RArrPair[A1, A2] =
  { val newA1Array = new Array[A1](length + 1)
    ...
    new RArrPair(newA1Array, newA2Array)
  }
  ...
  def init(implicit ct1: ClassTag[A1], ct2: ClassTag[A2]): RArrPair[A1, A2] = dropRight(1)

  def dropRight(n: Int)(implicit ct1: ClassTag[A1], ct2: ClassTag[A2]): RArrPair[A1, A2] =
  { val newLen = (length - n).max0
    ...
    new RArrPair[A1, A2](newA1s, newA2s)
  }
...
/** Companion object for [[RArrPair]], an un-specialised default [[Arr]] class for [[PairElem]]s contains factory methods for construction. */
object RArrPair
{ /** Factory apply method for constructing an [[RArrPair]] from the components of the pairs. */
  def apply[A1, A2](pairs: (A1, A2)*)(implicit ct1: ClassTag[A1], ct2: ClassTag[A2]): RArrPair[A1, A2] =
  { val len = pairs.length
    val a1s = new Array[A1](len)
    val a2s = new Array[A2](len)
    pairs.iForeach{(i, p) => a1s(i) = p._1; a2s(i) = p._2 }
    new RArrPair[A1, A2](a1s, a2s)
  }
  
  def apply[A1, A2](a1s: RArr[A1], a2s: RArr[A2]): RArrPair[A1, A2] = new RArrPair[A1, A2](a1s.arrayUnsafe, a2s.arrayUnsafe)

  /** Factory method for constructing an [[RArrPair]] from [[PairElem]]s. */
  def pairs[A1, A2](pairs: PairElem[A1, A2]*)(implicit ct1: ClassTag[A1], ct2: ClassTag[A2]): RArrPair[A1, A2] = {
    val len = pairs.length
    val a1s = new Array[A1](len)
    val a2s = new Array[A2](len)
  ...
  def fromBuff[A1, A2](buff: RPairBuff[A1, A2])(implicit ct1: ClassTag[A1], ct2: ClassTag[A2]): RArrPair[A1, A2] =
    new RArrPair[A1, A2](buff.b1Buffer.toArray, buff.b2Buffer.toArray)
...
{ override type ThisT = RPairBuff[B1, B2]
  ...
  def b1Buff: RBuff[B1] = new RBuff(b1Buffer)
  def toArr(implicit ct1: ClassTag[B1], ct2: ClassTag[B2]): RArrPair[B1, B2] = new RArrPair[B1, B2](b1Buffer.toArray, b2Buffer.toArray)
}
...
{ override type BuffT = RPairBuff[B1, B2]
  ...
  override def arrFromArrAndArray(b1Arr: RArr[B1], b2s: Array[B2]): RArrPair[B1, B2] = new RArrPair[B1, B2](b1Arr.arrayUnsafe, b2s)
  ...
  override def uninitialised(length: Int): RArrPair[B1, B2] = new RArrPair[B1, B2](new Array[B1](length), new Array[B2](length))

  override def indexSet(seqLike: RArrPair[B1, B2], index: Int, newElem: RPairElem[B1, B2]): Unit =
  ...
  override def arrFromBuffs(b1Buff: RBuff[B1], b2Buffer: ArrayBuffer[B2]): RArrPair[B1, B2] =
    new RArrPair[B1, B2](b1Buff.bufferUnsafe.toArray, b2Buffer.toArray)

  override def newBuff(length: Int): RPairBuff[B1, B2] = new RPairBuff[B1, B2](new ArrayBuffer[B1](length), new ArrayBuffer[B2](length))
  override def buffToSeqLike(buff: RPairBuff[B1, B2]): RArrPair[B1, B2] = new RArrPair[B1, B2](buff.b1Buffer.toArray, buff.b2Buffer.toArray)
}