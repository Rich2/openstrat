...
/** An [[Arr]] of pairs [[PairNoA1ParamElem]]. These classes allow convenient methods to map and filter on just one component of the pair. They and their
...
{ type ThisT <: PairArr[A1, A1Arr, A2, A]
  ...
  def a2Arr[A2Arr <: Arr[A2]](implicit build: MapBuilderArr[A2, A2Arr]): A2Arr = a2Array.mapArr(a2 => a2)

  /** Maps the first component of the pairs, dropping the second. */
  def a1Map[B, ArrB <: Arr[B]](f: A1 => B)(implicit builder: MapBuilderArr[B, ArrB]): ArrB = a1Arr.map(f)

  /** Maps the second component of the pairs, dropping the first. */
  def a2Map[B, ArrB <: Arr[B]](f: A2 => B)(implicit builder: MapBuilderArr[B, ArrB]): ArrB = a2Array.mapArr(f)
  ...
  def pairMap[B, ArrB <: Arr[B]](f: (A1, A2) => B)(implicit builder: MapBuilderArr[B, ArrB]): ArrB =
  { var i = 0
  ...
  def pairFlatMap[ArrB <: Arr[_]](f: (A1, A2) => ArrB)(implicit build: FlatBuilderArr[ArrB]): ArrB = {
  ...
  def optMapOnA1[B1, ArrB1 <: Arr[B1], B <: PairNoA1ParamElem[B1, A2], ArrB <: PairArr[B1, ArrB1, A2, B]](f: A1 => Option[B1])(implicit
    build: PairArrMapBuilder[B1, ArrB1, A2, B, ArrB]): ArrB =
  { val buff = build.newBuff()
    pairForeach { (a1, a2) => f(a1).foreach(b1 => buff.pairGrow(b1, a2)) }
    build.buffToSeqLike(buff)
...
trait PairArrCommonBuilder[B1, ArrB1 <: Arr[B1], B2, ArrB <: PairArr[B1, ArrB1, B2, _]] extends CommonBuilderSeqLike[ArrB]
{
...
trait PairArrMapBuilder[B1, ArrB1 <: Arr[B1], B2, B <: PairElem[B1, B2], ArrB <: PairArr[B1, ArrB1, B2, B]] extends
  PairArrCommonBuilder[B1, ArrB1, B2, ArrB] with MapBuilderArr[B, ArrB]
{
  type BuffT <: PairBuff[B1, B2, B]

  /** Builder for an Arr of the first element of the pair. */
  def b1ArrBuilder: MapBuilderArr[B1, ArrB1]
