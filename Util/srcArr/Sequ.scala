...
{ /** The final type of this object. */
  ...
  def map[B, ArrB <: Arr[B]](f: A => B)(implicit build: MapBuilderArr[B, ArrB]): ArrB =
  ...
  def optAllMap[B, ArrB <: Arr[B]](f: A => Option[B])(implicit build: MapBuilderArr[B, ArrB]): Option[ArrB] =
  { val res = build.uninitialised(length)
    var good = true
    var i = 0
  ...
  def optMap[B, ArrB <: Arr[B]](f: A => Option[B])(implicit build: MapBuilderArr[B, ArrB]): ArrB =
  ...
  def mapTo[B, BB <: SeqLike[B]](build: MapBuilderSeqLike[B, BB])(f: A => B): BB =
  { val res = build.uninitialised(length)
    var i = 0
  ...
  def indexMapTo[B, BB <: SeqLike[B]](iFrom: Int, iTo: Int, iStep: Int = 1)(f: A => B)(implicit build: MapBuilderSeqLike[B, BB]): BB =
  { val res = build.uninitialised(length)
    var ti = 0
   ...
   * classes. */
  def iMap[B, ArrB <: Arr[B]](f: (Int, A) => B)(implicit ev: MapBuilderArr[B, ArrB]): ArrB =
   ...
   * classes. */
  def iMap[B, ArrB <: Arr[B]](startindex: Int)(f: (Int, A) => B)(implicit ev: MapBuilderArr[B, ArrB]): ArrB =
  { val res = ev.uninitialised(length)
  ...
  def flatMap[ArrB <: Arr[_]](f: A => ArrB)(implicit ev: FlatBuilderArr[ArrB]): ArrB =
  ...
  def iFlatMap[ArrB <: Arr[_]](f: (Int, A) => ArrB)(implicit build: FlatBuilderArr[ArrB]): ArrB =
   ...
   * putting the collection element 2nd or last as seen for example in fold methods' (accumulator, element) => B signature. Ideally this method should
   * be overridden in sub classes. */
  def iFlatMap[ArrB <: Arr[_]](iInit: Int)(f: (Int, A) => ArrB)(implicit build: FlatBuilderArr[ArrB]): ArrB =
  { val buff: build.BuffT = build.newBuff()
    var count: Int = 0
    while (count < length)
  ...
  def zipMap[B, C, ArrC <: Arr[C]](operator: Sequ[B])(f: (A, B) => C)(implicit ev: MapBuilderArr[C, ArrC]): ArrC =
  ...
  def zipMap2[B, C, D, ArrD <: Arr[D]](operator1: Sequ[B], operator2: Sequ[C])(f: (A, B, C) => D)(implicit ev: MapBuilderArr[D, ArrD]): ArrD =
  ...
  def mapWithAcc[B, ArrB <: Arr[B], C](initC: C)(f: (C, A) => (B, C))(implicit ev: MapBuilderArr[B, ArrB]): ArrB =
  { val res = ev.uninitialised(length)
    var accC: C = initC
    iForeach({ (i, a) =>
  ...
  def eMap[B, ArrB <: Arr[B]](f: A => EMon[B])(implicit ev: MapBuilderArr[B, ArrB]): EMon[ArrB] =
  { val acc = ev.newBuff()
    var continue = true
    var count = 0
  ...
  def eMapLike[B, BB <: SeqLike[B]](f: A => EMon[B])(implicit ev: MapBuilderSeqLike[B, BB]): EMon[BB] =
  { val acc = ev.newBuff()
    var continue = true
    var count = 0
    var errs: StrArr = StrArr()
  ...
  def map2To1[B, ArrB <: Arr[B]](f: (A, A) => B)(implicit ev: MapBuilderArr[B, ArrB]): ArrB =
  { val res = ev.uninitialised(length)
    var count = 0
  ...
  def filter[ArrA <: Arr[A] @uncheckedVariance](f: A => Boolean)(implicit ev: MapBuilderArr[A, ArrA] @uncheckedVariance): ArrA =
  { val buff = ev.newBuff()
    foreach(a => onlyIf(f(a), ev.buffGrow(buff, a)))
    ev.buffToSeqLike(buff)
  }

  def filterNot[ArrA <: Arr[A] @uncheckedVariance](f: A => Boolean)(implicit ev: MapBuilderArr[A, ArrA] @uncheckedVariance): ArrA =
  { val buff = ev.newBuff()
    foreach(a => onlyIf(!f(a), ev.buffGrow(buff, a)))
    ev.buffToSeqLike(buff)
  }
  ...
  def flatToIterableMap[B, ArrB <: Arr[B]](f: A => Iterable[B])(implicit ev: MapBuilderArr[B, ArrB]): ArrB =
  ...
  def collect[B, BB <: Arr[B]](pf: PartialFunction[A, B])(implicit ev: MapBuilderArr[B, BB]): BB =
  { val acc = ev.newBuff()
  ...
  def mapCollectGoods[B, BB <: Arr[B]](f: A => EMon[B])(implicit ev: MapBuilderArr[B, BB]): BB =
  { val acc = ev.newBuff()
  ...
  def partition[ArrA <: Arr[A] @uncheckedVariance](f: A => Boolean)(implicit build: MapBuilderArr[A, ArrA] @uncheckedVariance): (ArrA, ArrA) =
