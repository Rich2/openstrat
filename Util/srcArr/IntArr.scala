...
{ def apply(input: Int*): IntArr = new IntArr(input.toArray)
  ...
  { override def typeStr: String = "Seq"
    ...
    override def fromExprOld(expr: Expr): EMon[IntArr] = expr match
...
{ type BuffT = IntBuff
  ...
  override def indexSet(seqLike: IntArr, index: Int, newElem: Int): Unit = seqLike.unsafeArray(index) = newElem
