...
{ /** This method and "fHead" removes the need for headOption in the majority of case. Use fHead when are interested in the tail value */
  ...
  def toArr[AA <: Arr[A]](implicit builder: MapBuilderArr[A, AA]): AA =
  ...
  def iMap[B, BB <: Arr[B]](f: (Int, A) => B)(implicit build: MapBuilderArr[B, BB]): BB =
  { var i = 0
  ...
  /** Specialised index with map to an immutable ArrBase of B. Note the function signature follows the foreach based convention of putting the
   *  collection element 2nd or last as seen for example in fold methods' (accumulator, element) => B signature. */
  def iMap[B, BB <: Arr[B]](count: Int)(f: (A, Int) => B)(implicit build: MapBuilderArr[B, BB]): BB =
  { var i = count
    val buff: build.BuffT = build.newBuff()
  ...
  def iFlatMap[B, BB <: Arr[B]](f: (Int, A) => BB)(implicit build: MapBuilderArr[B, BB]): BB =
  { var i = 0
    val buff: build.BuffT = build.newBuff()
  ...
  /** flatMaps over a traversable (collection / sequence) with a counter */
  def iFlatMap[B, BB <: Arr[B]](count: Int)(f: (Int, A) => BB)(implicit build: MapBuilderArr[B, BB]): BB =
  { var i = count
    val buff: build.BuffT = build.newBuff()
    thisIter.foreach{ el => f(i, el).foreach(buff.grow); i += 1 }
    build.buffToSeqLike(buff)
  ...
  /** maps to a [[Arr]] of B. */
  def mapArr[B, BB <: Arr[B]](f: A => B)(implicit ev: MapBuilderArr[B, BB]): BB = ev.iterMap[A](thisIter, f)

  /** flatMaps to a [[Arr]] of B. */
  def flatMapArr[BB <: Arr[_]](f: A => BB)(implicit ev: FlatBuilderArr[BB]): BB ={
    val buff = ev.newBuff()
