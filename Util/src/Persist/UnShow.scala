...
trait UnShow[+T] extends PersistBase
{
  ...
  def valuesFromStatements[ArrT <: SeqImut[T] @uncheckedVariance](sts: Statements)(implicit arrBuild: SeqBuild[T, ArrT] @uncheckedVariance): ArrT =
  ...
  def findUniqueTFromStatements[ArrT <: SeqImut[T] @uncheckedVariance](sts: Statements)(implicit arrBuild: SeqBuild[T, ArrT] @uncheckedVariance):
