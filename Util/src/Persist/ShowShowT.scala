...
/** A sub trait of the [[ShowTDec]] sub class where the type parameter of ShowT extends Show. This allows the ShowT type class to delegate to the Show
 ...
 * method delegate to the [[ShowTDec]] type class instance in the companion object. Potentially that can create initialisation order problems, but at the
 * very least it can increase compile times. */
trait ShowShowT[R <: ShowPrec] extends ShowTDec[R]
{
  ...
  override def showDecT(obj: R, way: ShowStyle, maxPlaces: Int, minPlaces: Int): String = obj.show(way, maxPlaces, 0)
