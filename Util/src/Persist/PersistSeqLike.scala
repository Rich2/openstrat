...
object AlphaSquareParenth
{
  ...
  {
    case AlphaBracketExpr(IdentifierLwToken(_, name), Arr2(SquareBlock(ts, _, _) , ParenthBlock(sts, _, _))) => Some((name, ts, sts))
    case _ => None
  }
}
...
object AlphaParenth
{
  ...
  {
    case AlphaBracketExpr(IdentifierLwToken(_, name), Arr1(ParenthBlock(sts, _, _))) => Some((name, sts))
    case _ => None
  }
}
...
abstract class Int2sArrPersist[A <: Int2Elem, M <: Int2sArr[A]](typeStr: String) extends IntNsArrPersist[A, M](typeStr)
{
