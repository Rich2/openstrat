...
{
  def unapply(expr: ClauseMemberExpr): Option[(String, Arr[Statement], Arr[Statement])] = expr match
  {
...
object AlphaParenth
{
  def unapply(expr: ClauseMemberExpr): Option[(String, Arr[Statement])] = expr match
  {
