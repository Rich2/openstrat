...
/** A convenience trait type class trait for persistence, that combines the [[ShowTDec]] and [[Unshow]] type classes. Most if not all final classes that
...
trait Persist[T] extends ShowTDec[T] with Unshow[T]