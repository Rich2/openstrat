...
sealed trait ERefsSpec[+A <: AnyRef] extends EMonBase[Arr[A]]
...
final case class GoodRefsSpec[+A <: AnyRef](value: Arr[A]) extends ERefsSpec[A] with GoodBase[Arr[A]]
...
case class BadRefsSpec[+A <: AnyRef](errs: Arr[String]) extends ERefsSpec[A] with BadBase[Arr[A]]
