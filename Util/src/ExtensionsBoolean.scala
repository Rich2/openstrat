...
class BooleanExtensions(val thisBool : Boolean) extends AnyVal// extends PersistSimple
{
  ...
  /** A safer alternative to using an "if" without an else */
   ...
   def errMap[A](fp: TextPosn, errStr: String, ifTrue: => A): EMonOld[A] = if (thisBool) Good[A](ifTrue) else fp.bad[A](errStr)
   def errFlatMap[A](fp: TextPosn, errStr: String, ifTrue: => EMonOld[A]): EMonOld[A] = if (thisBool) (ifTrue) else fp.bad[A](errStr)
