...
{ /** multiply the [[Multiple]] number with the operand. */
  ...
  def toArr[ArrA <: Arr[A]@uncheckedVariance](implicit build: MapBuilderArr[A, ArrA]@uncheckedVariance): ArrA =
...
{ type ThisT = MultipleArr[A]
  ...
  def toSinglesArr[ArrA <: Arr[A]](implicit build: MapBuilderArr[A, ArrA]): ArrA =
...
class MultipleArrMapBuilder[A](implicit ct: ClassTag[A]) extends MapBuilderArr[Multiple[A], MultipleArr[A]]
