...
object Multiple
{
  ...
  { override def typeStr: String = "Multiple"
    ...
    override def fromExpr(expr: Expr): ParseExcEither[Multiple[A]] =  expr match
    ...
    def fromArrExpr(inp: Arr[Expr]): ParseExcEither[RArr[Multiple[A]]] = inp.mapEither(fromExpr(_))

    /** Collection from [[Arr]] of [[pParse.Expr]]. */
    def collFromArrExpr[R](inp: Arr[Expr], builderColl: BuilderMap[A, R]): ParseExcEither[R] = fromArrExpr(inp).map(_.toColl(builderColl))

    /** Collection from [[Arr]] of [[pParse.Statement]]. */
    def collFromArrStatement[R](inp: Arr[Statement], builderColl: BuilderMap[A, R]): ExcEither[R] = collFromArrExpr(inp.map(_.expr), builderColl)
  }

  /** Collection from [[Arr]] of [[pParse.Expr]]. */
  def collFromArrExpr[Ae, A](inp: Arr[Expr])(using evA: Unshow[Ae], builderColl: BuilderMap[Ae, A]): ParseExcEither[A] =
    unshowEv.fromArrExpr(inp).map(_.toColl(builderColl))

  /** Collection from [[Arr]] of [[pParse.Statement]]. */
  def collFromArrStatement[A, R](inp: Arr[Statement])(using evA: Unshow[A], builderColl: BuilderMap[A, R]): ParseExcEither[R] =
