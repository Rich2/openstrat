...
case class Nat0yToken(startPosn: TextPosn, digitsStr: String) extends NatHexaToken
...
object Nat0yToken
{
  ...
  {
    def loop(rem: CharsOff, strAcc: String, intAcc: Long): EMon3[CharsOff, TextPosn, Nat0yToken] = rem match
    { case CharsOff0() => Good3(rem, tp.right(strAcc.length + 2), Nat0yToken(tp, strAcc))
      case CharsOff1Tail(Base32Char(c, i), tail) => loop(tail, strAcc + c, intAcc * 16 + i)
      case CharsOffHead(LetterChar(_)) => tp.bad3("Badly formed hexadecimal")
      case _ => Good3(rem, tp.addStr(strAcc), Nat0yToken(tp, strAcc))
    }

    rem match
    { case CharsOff3Tail('0', 'x', Base32Char(c, i), tail) => loop (tail, c.toString, i)
