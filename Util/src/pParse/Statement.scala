...
sealed trait Statement extends TextSpan
{
  ...
  final def errGet[A](implicit ev: PersistDec[A]): EMon[A] = ???
...
object Statement
{
  ...
  { private def ifEmptyTextPosn: TextPosn = TextPosn("Empty Statement Seq", 0, 0)
    ...
    def findSetting[T](settingStr: String)(implicit ev: PersistDec[T]): EMon[T] = ev.settingFromStatements(statements, settingStr)

    /** Find Setting of key type KT type T from this Arr[Statement]. Extension method. */
    def findKeySetting[KT, VT](key: KT)(implicit evST: Unshow[KT], ev: PersistDec[VT]): EMon[VT] = ev.keySettingFromStatements(statements, key)

    /** Searches for the setting of the correct type. If not found it searches for a unique setting / value of the correct type. */
    def findSettingOrUniqueT[T](settingStr: String)(implicit ev: PersistDec[T]): EMon[T] = findSetting[T](settingStr).goodOrOther(findUniqueT)

    /** Find idnetifier setting of value type T from this Arr[Statement] or return the default value parameter. Extension method */
    def findSettingElse[A](settingStr: String, elseValue: A)(implicit ev: PersistDec[A]): A = findSetting[A](settingStr).getElse(elseValue)
    ...
    def findSettingDbl(settingStr: String): EMon[Double] = ShowT.doublePersistEv.settingFromStatements(statements, settingStr)
