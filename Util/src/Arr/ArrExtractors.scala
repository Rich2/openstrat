...
{ /** Extractor for empty Arr[A]. Arr[A] is an immutable covariant Array based collection. */
  def unapply[A](arr: RArr[A]): Boolean = arr.ssLength == 0
}
...
{ /** Extractor for Arr[A] of length == 1. Arr[A] is an immutable covariant Array based collection. */
  def unapply[A](arr: RArr[A]): Option[A] = arr.ssLength match
  { case 1 => Some(arr(0))
    case _ => None
  }
}
...
{ /** Extractor for Arr[A] of length == 2. Arr[A] is an immutable covariant Array based collection. */
  def unapply[A](arr: RArr[A]): Option[(A, A)] = ife(arr.ssLength == 2, Some((arr(0), arr(1))), None)
}
...
{ /** Extractor for Arr[A] of length == 3, Arr[A] is an immutable covariant Array based collection. */
  def unapply[A](arr: RArr[A]): Option[(A, A, A)] = arr.ssLength match
...
{ /** Extractor for Arr[A] of length == 4, Arr[A] is an immutable covariant Array based collection. */
  def unapply[A](arr: RArr[A]): Option[(A, A, A, A)] = arr.ssLength match
...
{ /** Extractor for Arr[A] of length == 5, Arr[A] is an immutable covariant Array based collection. */
  def unapply[A](arr: RArr[A]): Option[(A, A, A, A, A)] = arr.ssLength match
...
{ /** Extractor for Arr[A] of length == 6, Arr[A] is an immutable covariant Array based collection. */
  def unapply[A](arr: RArr[A]): Option[(A, A, A, A, A, A)] = arr.ssLength match
...
{ /** Extractor method for Arr of length >= 2. Optionally Returns a [[Tuple3]] of the first 2 elements and the tail. */
  def unapply[A](arr: RArr[A])(implicit ct: ClassTag[A]): Option[(A, A, RArr[A])] = ife(arr.ssLength >= 2, Some((arr(0), arr(1), arr.drop(2))), None)
}

object Arr3Tail
{ /** Extractor method for Arr of length >= 3. Optionally Returns a [[Tuple4]] of the first 3 elements and the tail. */
  def unapply[A](arr: RArr[A])(implicit ct: ClassTag[A]): Option[(A, A, A, RArr[A])] = ife(arr.ssLength >= 3, Some((arr(0), arr(1), arr(2), arr.drop1)), None)
}
...
object ArrHead3
{ /** Extractor for the head of an Arr, immutable covariant Array based collection. The tail can be any length. */
  def unapply[A](arr: RArr[A]): Option[(A, A, A)] = ife(arr.ssLength >= 3, Some((arr(0), arr(1), arr(2))), None)
}

object ArrHead4
{ /** Extractor for the head of an Arr, immutable covariant Array based collection. The tail can be any length. */
  def unapply[A](arr: RArr[A]): Option[(A, A, A, A)] = ife(arr.ssLength >= 4, Some((arr(0), arr(1), arr(2), arr(3))), None)
}
...
object GoodArr2
{ def unapply[A](eArr: EMon[RArr[A]]): Option[(A, A)] = eArr.foldErrs (g => if (g.ssLength == 2) Some((g(0), g(1))) else None)(errs => None)
}
...
{ override def apply(index: Int)(implicit arr: RArr[A]) = arr(index + offset0)
  ...
  def length(implicit arr: RArr[A]): Int = arr.ssLength - offset0
  ...
  { var count = 0
    ...
    while (offset0 + count < arr.ssLength & continue)
  ...
  def ifHead(f: A => Boolean)(implicit arr: RArr[A]): Boolean = (arr.ssLength > offset0) & f(arr(offset0))
  ...
  def headFold[B](emptyValue: => B)(f: (A, ArrOff[A]) => B)(implicit arr: RArr[A]): B =
    if (arr.ssLength > offset0) f(arr(offset0), drop1) else emptyValue
}
