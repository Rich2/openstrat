...
class CharsOff(val offset0: Int) extends AnyVal with ArrBaseOff[Char, Chars]
{
  ...
  def length(implicit chars: Chars): Int = chars.elemsLen - offset0
  def span(p: Char => Boolean)(implicit array: Chars): (Chars, CharsOff) =
  {
    var count = 0
    var continue = true
    while (offset0 + count < array.elemsLen & continue)
    {
  ...
  def ifHead(f: Char => Boolean)(implicit chars: Chars) : Boolean = (chars.elemsLen > offset0) &
    f(chars(offset0))
}
...
case object CharsOff0 { def unapply(inp: CharsOff)(implicit chars: Chars): Boolean = chars.elemsLen - inp.offset0 <= 0 }
...
{ /** Extractor for immutable heapless iterator for Chars with length == 1. */
  def unapply(inp: CharsOff)(implicit chars: Chars): Option[Char] = ife(chars.elemsLen - inp.offset0 == 1, Some(chars(inp.offset0)), None)
}
...
{ /** Extractor for immutable heapless iterator for Chars with length == 2. */
  def unapply(inp: CharsOff)(implicit chars: Chars): Option[(Char, Char)] =
    ife(chars.elemsLen - inp.offset0 == 2, Some((chars(inp.offset0), chars(inp.offset1))), None)
}
...
{ /** Extractor for immutable heapless iterator for Chars with length == 3. */
  def unapply(inp: CharsOff)(implicit chars: Chars): Option[(Char, Char, Char)] =
    ife(chars.elemsLen - inp.offset0 == 3, Some((chars(inp.offset0), chars(inp.offset1), chars(inp.offset2))), None)
}
...
{ /** Extractor for immutable heapless iterator for Chars with length == 4. */
  def unapply(inp: CharsOff)(implicit chars: Chars): Option[(Char, Char, Char, Char)] =
    ife(chars.elemsLen - inp.offset0 == 4, Some((chars(inp.offset0), chars(inp.offset1), chars(inp.offset2), chars(inp.offset3))), None)
}
...
{ /** Extractor for the first element, for immutable heapless iterator for Chars with length >= 1. Use this when you don't care about the tail. */
  ...
  ife(chars.elemsLen - inp.offset0 >= 1, Some(chars(inp.offset0)), None)
}
...
{ /** Extractor for immutable heapless iterator for Chars with at l element. */
  ...
  ife(chars.elemsLen - inp.offset0 >= 1, Some((chars(inp.offset0), inp.drop1)), None)
}

object CharsOff2Tail
{ def unapply(inp: CharsOff)(implicit array: Chars): Option[(Char, Char, CharsOff)] =
    ife(array.elemsLen - inp.offset0 >= 2, Some((array(inp.offset0), (array(inp.offset1)), inp.drop2)), None)
}

object CharsOff3Tail
{ def unapply(inp: CharsOff)(implicit array: Chars): Option[(Char, Char, Char, CharsOff)] =
    ife(array.elemsLen - inp.offset0 >= 3, Some((array(inp.offset0), array(inp.offset1), array(inp.offset2), inp.drop3)), None)
}

object CharsOff4Tail
{ def unapply(inp: CharsOff)(implicit array: Chars): Option[(Char, Char, Char, Char, CharsOff)] =
  ife(array.elemsLen - inp.offset0 >= 4,
