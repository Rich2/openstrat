 ...
 *  you control should go in the companion object of BB. This is different from the related [[SeqBuild]][BB] type class where the instance
 *  should go into the B companion object. */
trait SeqFlatBuild[ArrT <: SeqImut[_]] extends SeqBuildCommon[ArrT]

/** Companion object for ArrTFlatBuilder, contains implicit instances for atomic value classes. */
object SeqFlatBuild extends ArrFlatBuildLowPriority
{ implicit val intsImplicit: SeqFlatBuild[Ints] = IntsBuild
  implicit val dblsImplicit: SeqFlatBuild[Dbls] = DblsBuild
  implicit val longsImplicit: SeqFlatBuild[Longs] = LongsBuild
  implicit val floatImplicit: SeqFlatBuild[Floats] = FloatsBuild
  implicit val booleansImplicit: SeqFlatBuild[Booleans] = BooleansBuild
}
...
{ /** This is the fall back builder implicit for Arrs that do not have their own specialist ArrBuildBase classes. It is placed in this low priority trait
 ...
 implicit def anyImplicit[A](implicit ct: ClassTag[A], @unused notA: Not[ValueNElem]#L[A]): SeqFlatBuild[Arr[A]] = new AnyBuild[A]
}