...
{ /** The final type of this object. */
  ...
  def map[B, ArrB <: SeqImut[B]](f: A => B)(implicit ev: ArrTBuilder[B, ArrB]): ArrB =
  ...
  def flatMap[ArrB <: SeqImut[_]](f: A => ArrB)(implicit ev: ArrTFlatBuilder[ArrB]): ArrB =
  {
  ...
  def zipMap[B, C, ArrC <: SeqImut[C]](operator: SeqGen[B])(f: (A, B) => C)(implicit ev: ArrTBuilder[C, ArrC]): ArrC =
  ...
  def zipMap2[B, C, D, ArrD <: SeqImut[D]](operator1: SeqGen[B], operator2: SeqGen[C])(f: (A, B, C) => D)(implicit ev: ArrTBuilder[D, ArrD]): ArrD =
  ...
  def iMap[B, ArrB <: SeqImut[B]](f: (A, Int) => B)(implicit ev: ArrTBuilder[B, ArrB]): ArrB =
  { val res = ev.newArr(elemsNum)
  ...
  def iFlatMap[ArrB <: SeqImut[_]](f: (A, Int) => ArrB)(implicit build: ArrTFlatBuilder[ArrB]): ArrB =
  ...
  /** Specialised flatMap with index to an immutable Arr. */
  def iFlatMap[ArrB <: SeqImut[_]](iInit: Int = 0)(f: (A, Int) => ArrB)(implicit build: ArrTFlatBuilder[ArrB]): ArrB =
  { val buff: build.BuffT = build.newBuff()
    var count: Int = 0
    while (count < elemsNum)
  ...
  def mapWithAcc[B, ArrB <: SeqImut[B], C](initC: C)(f: (A, C) => (B, C))(implicit ev: ArrTBuilder[B, ArrB]): ArrB =
  { val res = ev.newArr(elemsNum)
    var accC: C = initC
    iForeach { (a, i) =>
  ...
  def eMap[B, ArrB <: SeqImut[B]](f: A => EMon[B])(implicit ev: ArrTBuilder[B, ArrB]): EMon[ArrB] =
  { val acc = ev.newBuff()
    var continue = true
    var count = 0
  ...
  def map2To1[B, ArrB <: SeqImut[B]](f: (A, A) => B)(implicit ev: ArrTBuilder[B, ArrB]): ArrB =
  { val res = ev.newArr(elemsNum)
    var count = 0
  ...
  def filter[ArrA <: SeqImut[A] @uncheckedVariance](f: A => Boolean)(implicit ev: ArrTBuilder[A, ArrA] @uncheckedVariance): ArrA =
  { val buff = ev.newBuff()
    foreach(a => oif(f(a), ev.buffGrow(buff, a)))
    ev.buffToArr(buff)
  }

  def filterNot[ArrA <: SeqImut[A] @uncheckedVariance](f: A => Boolean)(implicit ev: ArrTBuilder[A, ArrA] @uncheckedVariance): ArrA =
  { val buff = ev.newBuff()
    foreach(a => oif(!f(a), ev.buffGrow(buff, a)))
    ev.buffToArr(buff)
  }
  ...
  def iterFlatMap[B, ArrB <: SeqImut[B]](f: A => Iterable[B])(implicit ev: ArrTBuilder[B, ArrB]): ArrB =
  ...
  def collect[B, BB <: SeqImut[B]](pf: PartialFunction[A, B])(implicit ev: ArrTBuilder[B, BB]): BB =
  { val acc = ev.newBuff()
  ...
  def mapCollectGoods[B, BB <: SeqImut[B]](f: A => EMon[B])(implicit ev: ArrTBuilder[B, BB]): BB =
  { val acc = ev.newBuff()
