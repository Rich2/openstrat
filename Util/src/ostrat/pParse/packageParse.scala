...
package object pParse
{
  ...
  def getStatements(input: String, inputSourceName: String): EMonArr[Statement] = TokensFind(input)(inputSourceName).flatMap(GetStatements(_))
  /** Returns an EMon of a sequence of Statements from a String. */
  def stringToStatements(input: String): EMonArr[Statement] = stringToTokens(input).flatMap(GetStatements(_)).map(_.toArr)
  /** Max numbers for long and hexidecimal formats needs to be implemented */
  def stringToTokens(srcStr: String): EMon[List[Token]] = TokensFind(srcStr).fromString
}