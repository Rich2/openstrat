...
package object pParse
{
  ...
  def getStatements(input: String, inputSourceName: String): EMonArr[Statement] =
    TokensFind(input)(inputSourceName).map(g => g.toList).flatMap(GetStatements(_))
  /** Returns an EMon of a sequence of Statements from a String. */
  def stringToStatements(input: String): EMonArr[Statement] =
    stringToTokens(input).map(g => g.toList).flatMap(GetStatements(_)).map(_.toArr)
