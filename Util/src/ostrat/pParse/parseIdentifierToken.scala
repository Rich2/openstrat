...
object parseIdentifierToken
{
  def apply(remOff: CharsOff, tpStart: TextPosn)(implicit charArr: Chars): EMon3[CharsOff, TextPosn, Token] =
  {
    ...
    { case CharsOff0() => Good3(remOff, tp, IdentifierMaybeTrigToken(tpStart, acc.toString))
      case CharsOff2Plus('_', LetterOrDigitChar(_))  => { acc.append('_'); lowerLoop(remOff.drop1, tp.right1) }
      ...
      case CharsOff1Plus(_) => Good3(remOff, tp, IdentifierMaybeTrigToken(tpStart, acc.toString))
    }
    ...
    { case CharsOff0() => Good3(remOff, tp, IdentifierMaybeHexaToken(tpStart, acc.toString))
      case CharsOff2Plus('_', LetterOrDigitChar(_))  => { acc.append('_'); upperLoop(remOff.drop1, tp.right1) }
      case CharsOff2Plus('_', '_') => tp.right1.bad("Consecutive underscores in Identifier not allowed.")
      ...
      case CharsOff1Plus(_) => Good3(remOff, tp, IdentifierMaybeHexaToken(tpStart, acc.toString))
    }
    ...
    { case CharsOff0() => Good3(remOff, tp, IdentifierUpperOnlyToken(tpStart, acc.toString))
      ...
      case CharsOff2Plus('_', '_') => tp.right1.bad("Consecutive underscores in Identifier not allowed.")
      case CharsOff1Tail(LetterOrDigitChar(c), tail) => { acc.append(c); upperLoop(tail, tp.right1) }
      case CharsOff1Plus(_) => Good3(remOff, tp, IdentifierUpperOnlyToken(tpStart, acc.toString))
    }
    ...
    remOff match
    {
      ...
      case CharsOff2Plus('_', LetterOrDigitChar(_)) => { acc.append('_'); lowerLoop(remOff.drop1, tpStart.right1) }
