...
trait ArrHomoInt4[A <: HomoInt4] extends Any with ArrHomoIntN[A]
{
...
trait ProductI4sBuff[A <: HomoInt4, M <: ArrHomoInt4[A]] extends Any with ArrBuffHomoInts[A, M]
{ override def append(newElem: A): Unit = { buffer.append(newElem._1).append(newElem._2).append(newElem._3).append(newElem._4); ()}
}

abstract class ProductI4sCompanion[A <: HomoInt4, M <: ArrHomoInt4[A]]
{ val factory: Int => M
...
abstract class ProductI4sBuilder[A <: HomoInt4, M <: ArrHomoInt4[A]](typeStr: String) extends ProductIntsBuilder[A, M](typeStr)
{
