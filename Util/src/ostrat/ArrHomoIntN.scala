...
trait ArrHomoIntN[A] extends Any with ArrHomo[A]
{ type ThisT <: ArrHomoIntN[A]
  def array: Array[Int]
...
trait ArrBuffHomoInts[A, M <: ArrHomoIntN[A]] extends Any with ArrBuffHomo[A, M]
...
abstract class ProductIntsBuilder[A, M <: ArrHomoIntN[A]](typeStr: String) extends ArrHomoBuilder[A, M](typeStr)
{ type VT = Int
