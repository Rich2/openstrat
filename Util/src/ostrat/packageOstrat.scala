...
{ import collection.mutable.ArrayBuffer, reflect.ClassTag
  ...
  def iDblToMap[A, AA <: SeqImut[A]](iFrom: Double, iTo: Double, iStep: Double = 1)(f: Double => A)(implicit ev: SeqBuild[A, AA]): AA =
  ...
  def iToMap[A, AA <: SeqImut[A]](iFrom: Int, iTo: Int, iStep: Int = 1)(f: Int => A)(implicit ev: SeqBuild[A, AA]): AA =
  ...
  def iUntilMap[A, AA <: SeqImut[A]](iFrom: Int, iUntil: Int, iStep: Int = 1)(f: Int => A)(implicit ev: SeqBuild[A, AA]): AA =
  ...
  def iToFlatMap[AA <: SeqImut[_]](iFrom: Int, iTo: Int, iStep: Int = 1)(f: Int => AA)(implicit ev: SeqFlatBuild[AA]): AA =
  { val buff = ev.newBuff()
  ...
  def ijUntilMap[A, AA <: SeqImut[A]](iFrom: Int, iUntil: Int, iStep: Int = 1)(jFrom: Int, jUntil: Int, jStep: Int = 1)(f: (Int, Int) => A)
                                     (implicit ev: SeqBuild[A, AA]): AA = ijToMap[A, AA](iFrom, ife(iStep > 0, iUntil - 1, iUntil + 1), iStep)(jFrom, jUntil - 1, jStep)(f)

  /** ijToMap where i and j have identical ranges. */
  def ijSameToMap[A, AA <: SeqImut[A]](nFrom: Int, nTo: Int, nStep: Int = 1)(f: (Int, Int) => A)(implicit ev: SeqBuild[A, AA]): AA =
  ...
  def ijkToMap[A, AA <: SeqImut[A]](iFrom: Int, iTo: Int, iStep: Int = 1)(jFrom: Int, jTo: Int, jStep: Int = 1)(kFrom: Int, kTo: Int, kStep: Int = 1)
                                   (f: (Int, Int, Int) => A)(implicit ev: SeqBuild[A, AA]): AA =
  { val iLen = (iTo - iFrom + iStep).max(0) / iStep
    val jLen = (jTo - jFrom + jStep).max(0) / jStep
