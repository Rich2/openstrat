...
{  
  ...
  class ArrayRefPersist[A <: AnyRef](ev: Persist[A]) extends PersistSeqLike[A, Array[A]](ev)
  {
    ...
    {
      case AlphaBracketExpr(IdentLowerOnlyToken(_, typeName), Refs1(ParenthBlock(sts, _, _))) if typeStr == typeName => ??? // fromParameterStatements(sts)
      case AlphaBracketExpr(IdentLowerOnlyToken(fp, typeName), _) => fp.bad(typeName -- "does not equal" -- typeStr)
  ...
  implicit def someToPersist[A](implicit ev: Persist[A]): Persist[Some[A]] = new Persist[Some[A]]
  {
    ...
    {
      case AlphaBracketExpr(IdentLowerOnlyToken(_, "Some"), Refs1(ParenthBlock(Refs1(hs), _, _))) => ev.fromExpr(hs.expr).map(Some(_))
  ...
  { def show(obj: Float): String = obj.toString
    ...
    { case IntDeciToken(_, i) => Good(i.toFloat)
      case PreOpExpr(op, IntDeciToken(_, i)) if op.srcStr == "+" => Good(i.toFloat)
      case PreOpExpr(op, IntDeciToken(_, i)) if op.srcStr == "-" => Good(-(i.toFloat))
  ...
  { def show(obj: Double): String = obj.toString      
    ...
    { case IntDeciToken(_, i) => Good(i.toDouble)
      case PreOpExpr(op, IntDeciToken(_, i)) if op.srcStr == "+" => Good(i.toDouble)
      case PreOpExpr(op, IntDeciToken(_, i)) if op.srcStr == "-" => Good(-(i.toDouble))
  ...
  { override def showSemi(thisArray: Array[Int]): String = thisArray.map(evA.showComma(_)).semiFold
    ...
    { case SemicolonToken(_) => Good(Array[Int]())
      case AlphaBracketExpr(IdentLowerOnlyToken(_, "Seq"), Refs2(SquareBlock(ts, _, _), ParenthBlock(sts, _, _))) => ???
