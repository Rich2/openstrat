...
{ import collection.mutable.ArrayBuffer, reflect.ClassTag
  ...
  def iDblToMap[A, AA <: Arr[A]](iFrom: Double, iTo: Double, iStep: Double = 1)(f: Double => A)(using ev: BuilderArrMap[A, AA]): AA =
  ...
  def iToMap[A, AA <: Arr[A]](iFrom: Int, iTo: Int, iStep: Int = 1)(f: Int => A)(using ev: BuilderArrMap[A, AA]): AA =
  ...
  def iUntilMap[A, AA <: Arr[A]](iFrom: Int, iUntil: Int, iStep: Int = 1)(f: Int => A)(using ev: BuilderArrMap[A, AA]): AA =
  ...
  def iToFlatMap[AA <: Arr[?]](iFrom: Int, iTo: Int, iStep: Int = 1)(f: Int => AA)(using ev: BuilderArrFlat[AA]): AA =
  { val buff = ev.newBuff()
  ...
  def iToFlatMap[AA <: Arr[?]](iTo: Int)(f: Int => AA)(using ev: BuilderArrFlat[AA]): AA =
  { val buff = ev.newBuff()
   ...
   * 1. */
  def iUntilFlatMap[AA <: Arr[?]](iFrom: Int, iUntil: Int, iStep: Int = 1)(f: Int => AA)(using ev: BuilderArrFlat[AA]): AA =
  { val buff = ev.newBuff()
  ...
  def iUntilFlatMap[AA <: Arr[?]](iUntil: Int)(f: Int => AA)(using ev: BuilderArrFlat[AA]): AA =
  { val buff = ev.newBuff()
  ...
  def iLoopToMap[A, AA <: Arr[A]](loopEnd: Int, loopStart: Int = 0)(iFrom: Int, iTo: Int, iStep: Int = 1)(f: Int => A)(using ev: BuilderArrMap[A, AA]): AA =
  { val iLen = (iTo + 1).max(0)
    val res: AA = ev.uninitialised(iLen)
    var index = 0
  ...
  def ijkToMap[A, AA <: Arr[A]](iFrom: Int, iTo: Int, iStep: Int = 1)(jFrom: Int, jTo: Int, jStep: Int = 1)(kFrom: Int, kTo: Int, kStep: Int = 1)
    (f: (Int, Int, Int) => A)(using ev: BuilderArrMap[A, AA]): AA =
  { val iLen = (iTo - iFrom + iStep).max(0) / iStep
    val jLen = (jTo - jFrom + jStep).max(0) / jStep
  ...
  implicit class RangeExtensions(range: Range)
  {
    ...
    def mapArr[B <: ValueNElem , M <: Arr[B]](f: Int => B)(using build: BuilderArrMap[B, M]): M =
