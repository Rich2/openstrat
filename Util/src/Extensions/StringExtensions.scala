...
class StringImplicit(val thisString: String) extends AnyVal //extends PersistStr
{
  ...
  def parseStatements: ERefs[pParse.Statement] = parseTokens.flatMapRefs(pParse.astParse(_))//.toOld
  ...
  def findType[A: Persist]: EMon[A] = thisString.parseStatements.toOld.flatMapOld(_.findType[A])

  /** Finds Statement of type A. */
  def findTypeElse[A: Persist](elseValue: => A): A = findType[A].getElse(elseValue)
  def findInt: EMon[Int] = thisString.parseStatements.toOld.flatMapOld(_.findInt)
  def findDouble: EMon[Double] = thisString.parseStatements.toOld.flatMapOld(_.findDouble)
  def findBoolean: EMon[Boolean] = thisString.parseStatements.toOld.flatMapOld(_.findBoolean)
  def findTypeIndex[A: Persist](index: Int): EMon[A] = thisString.parseStatements.toOld.flatMapOld(_.findTypeIndex[A](index))
  ...
  def findIntArray: EMon[Array[Int]] = thisString.parseStatements.toOld.flatMapOld(_.findIntArray)

  def findSett[A: Persist](settingStr: String): EMon[A] = thisString.parseStatements.toOld.flatMapOld(_.findSett[A](settingStr))
  def findSettElse[A: Persist](settingStr: String, elseValue: A): A = findSett[A](settingStr).getElse(elseValue)
  def findIntSett(settingStr: String): EMon[Int] = thisString.parseStatements.toOld.flatMapOld(_.findIntSett(settingStr))
  def findIntSettElse(settingStr: String, elseValue: Int): Int = findIntSett(settingStr).getElse(elseValue)
  def findDoubleSett(settingStr: String): EMon[Double] = thisString.parseStatements.toOld.flatMapOld(_.findDoubleSett(settingStr))
  def findDoubleSettElse(settingStr: String, elseValue: Double): Double = findDoubleSett(settingStr).getElse(elseValue)
  def findBooleanSett(settingStr: String): EMon[Boolean] = thisString.parseStatements.toOld.flatMapOld(_.findBooleanSett(settingStr))
