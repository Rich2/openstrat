...
{ /** This method and "fHead" removes the need for headOption in the majority of case. Use fHead when are interested in the tail value */
  ...
  def toImut[AA <: SeqImut[A]](implicit bu: ArrTBuilder[A, AA]): AA =
  ...
  /** Maps over and  */
  def iMap[B, BB <: SeqImut[B]](f: (A, Int) => B, count: Int = 0)(implicit build: ArrTBuilder[B, BB]): BB =
  { var i = count
  ...
  def iFlatMap[B, BB <: SeqImut[B]](f: (A, Int) => BB, count: Int = 0)(implicit build: ArrTBuilder[B, BB]): BB =
  { var i = count
    val buff: build.BuffT = build.newBuff()
  ...
  def mapArr[B, BB <: SeqImut[B]](f: A => B)(implicit ev: ArrTBuilder[B, BB]): BB = ev.iterMap[A](thisIter, f)
}
