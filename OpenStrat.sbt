...
def proj(name: String): Project = Project(name, file("SbtWork/" + name))
...
lazy val GraphicJvm = proj("GraphicJvm").dependsOn(MacrosJvm).settings(graphicSettings).settings(
...
lazy val GraphicJs = proj("GraphicJs").dependsOn(MacrosJs).settings(graphicSettings).enablePlugins(ScalaJSPlugin).settings(libraryDependencies += "org.scala-js" %%% "scalajs-dom" % "0.9.6")
