...
object Australasia extends Area1("Australasia", -23 ll 130)
{
   ...
   val capeLeeuwin = deg(-34.36, 115.13)
   val wAustralia = deg(-22.58, 113.95)
   val eightyMile = deg(-19.41, 121.24)
   val couloumbPoint = deg(-17.30, 122.12)
   val drysdaleRiver = deg(-13.77, 126.95)
   val victoriaMouth = deg(-15.13, 129.65)
   val thamarrurr = deg(-14.42, 129.36)
   val coxPeninsular = deg(-12.41, 130.64)
   val nAustralia = deg(-12.01, 133.56)
   val eastArnhem = deg(-12.31, 136.92)
   val limmen = deg(-14.73, 135.36)
   val karumba = deg(-17.52, 140.8)
   val nQueensland = deg(-11, 142.43)
   
   val nKennedy = deg(-14.49, 143.95)
   val capeMelville = deg(-14.17, 144.51)
   val coolbie =deg(-18.86, 146.27)
   val harveyBay = deg(-25.29, 152.89)
   val brisbane = deg(-27.05, 153.03)
   val byronBay = deg(-28.64, 153.62)
   val seAustralia = deg(-37.4, 149.58)
   
   val wilsonsProm = deg(-39.12, 146.38)
   val barwonHeads = deg(-38.27, 144.53)
   val capeOtway = deg(-38.85, 143.51)
   val portMacdonnell = deg(-38.06, 140.66)
   val carpenterRocks = deg(-37.89, 140.28)
   val capeJaffa = deg(-36.96, 139.67)
   val hardwicke = deg(-34.91, 137.46)
   val portAugusta = deg(-32.53, 137.77)
   val sleaford = deg(-34.92, 135.64)
   val smokyBay = deg(-32.52, 133.86)
   val yalata = deg(-31.35, 131.21)
   val nuytsland1 = deg(-32.96, 124.33)
   val nuytsland2 = deg(-33.86, 123.63)
   val windyHarbour = deg(-34.84, 116)
   
   val australia = Area2("Australia", deg(-24.45, 134.47), desert,
         ...
         wilsonsProm, barwonHeads, capeOtway, portMacdonnell, carpenterRocks, carpenterRocks,
         hardwicke, portAugusta, sleaford, smokyBay, yalata, nuytsland1, nuytsland2, windyHarbour) 
...
//   }
  ...
  // override val gridMaker = E80Empty
   ...
   override val a2Seq = ArrOld(sumatra, borneo, sulawesi, javaIsland, newGuinea, australia, newZealandNIsland, newZealandSIsland)
