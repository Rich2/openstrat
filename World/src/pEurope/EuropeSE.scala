...
{ val odessa = 46.48 ll 30.74
  ...
  val latLongs: LatLongs = LatLongs(Alpsland.monfalcone, Alpsland.zagreb, odessa, burgas, istanbul, seddElBahr, thessalonika, nEuboea, sEuboea,
...
object Crimea extends WldArea2("Crimea", 45.33 ll 34.15, plain)
{ val henichesk = 46.17 ll 34.82
  ...
  val latLongs: LatLongs = LatLongs(henichesk, kerch, crimeaS, crimeaW, crimeaNW)
}

object Ukraine extends WldArea2("Ukraine", 50 ll 24, plain)
{ val caspianWLat = 44.53.north
  ...
  val latLongs: LatLongs = LatLongs(Baltland.southEast, caspianW, asiaMinorNM, llich, rostov, Crimea.henichesk, Crimea.crimeaNW, koblev,
     Balkans.odessa, Polandia.cenEast)
}

object Anatolia extends WldArea2("AsiaMinor", 39.46 ll 33.07, hills)
{ val sinopeN = 42.09 ll 34.99
  ...
  val latLongs: LatLongs = LatLongs( Balkans.istanbul, sinopeN, surmene, cizre, damascus, eilat, sSinai, SaharaEast.suez, SaharaEast.portSaid, eGaza,
     yukanbumaz, bodrum, Balkans.seddElBahr)
}

object Caucasus extends WldArea2("Caucasus", 42.0 ll 45.0, hills)
{ val blackSeaE = 41.84 ll 41.77
  ...
  val latLongs: LatLongs = LatLongs(Anatolia.surmene, blackSeaE, Ukraine.asiaMinorNM, Ukraine.caspianW, sumqayit, baku, sangachal, caspianSW,
     Anatolia.cizre)
}